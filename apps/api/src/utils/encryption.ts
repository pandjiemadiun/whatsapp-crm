...
import { FieldDecryptionError, type DecryptFieldContext } from '../errors/FieldDecryptionError.js';
...
export async function getEncryptionKey(): Promise<Buffer | null> {
  ...
  try {
    ...
    console.warn('[encryption] Failed to read key from Platform Config:', (error as Error).message);
  }
  ...
  if (workerUrl && token) {
    ...
    try {
      ...
      console.warn('[encryption] Failed to fetch key from Cloudflare Worker:', (error as Error).message);
    }
  }
 ...
 */
export function encryptField(plaintext: string | null | undefined, key: Buffer | null): string | null {
 ...
 */
export function decryptField(encrypted: string | null | undefined, key: Buffer | null, ctx?: DecryptFieldContext): string | null {
  if (!encrypted || encrypted === null) return null;
 ...
 */
export function hashField(value: string | null | undefined, key: Buffer | null): string | null {
...
export function setTestKey(key: Buffer) {
  cachedKey = key;
  keyExpiry = Date.now() + KEY_TTL_MS;
}
