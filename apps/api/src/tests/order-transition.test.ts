...
import {
  ...
  ALLOWED_TRANSITIONS,
  CONFIRMED_STATUSES,
} from '../business/order-transition.js';
...
async function createOrder(status: string): Promise<Order> {
...
test('T17. draft → completed throws InvalidOrderTransitionError', async () => {
  ...
  await assert.rejects(
    () => transitionOrder(order.id, 'completed', { tx: prisma, actor: 'test' }),
    (err: unknown) => err instanceof InvalidOrderTransitionError,
  );
});
...
test('T18. cancelled → paid throws InvalidOrderTransitionError (terminal)', async () => {
  ...
  await assert.rejects(
    () => transitionOrder(order.id, 'paid', { tx: prisma, actor: 'test' }),
    (err: unknown) => err instanceof InvalidOrderTransitionError,
  );
});
...
test('T21. Order not found throws ApiError', async () => {
  ...
  await assert.rejects(
    () => transitionOrder('nonexistent-id', 'confirmed', { tx: prisma, actor: 'test' }),
