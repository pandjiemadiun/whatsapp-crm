...
import { decideRoute, RouteContext, RouteDecision, ConfirmedItemLike, ActiveOrderLike } from '../business/route-decider.js';
...
const rawDataset: any[] = JSON.parse(readFileSync(goldenPath, 'utf-8'));
// Filter: only single-turn cases go to golden runner; multi-turn handled by multi-turn-invariants.test.ts
const dataset: GoldenCase[] = rawDataset.filter(c => !c.turns);
const TOTAL_CASES = dataset.length;

function buildTestCtx(testCase: GoldenCase, convId: string): RouteContext {
  return {
...
async function buildTestCtxNormalized(testCase: GoldenCase, convId: string): Promise<RouteContext> {
  const ctx = buildTestCtx(testCase, convId);
  const { normalizeMessage } = await import('../services/chat/normalizer.js');
  const productNames: string[] = (testCase.context.cart || []).map(c => c.product);
  const { normalized } = await normalizeMessage(testCase.input, ctx.storeId, productNames);
  ctx.lowerMsg = normalized.toLowerCase();
  return ctx;
}

async function setupDb(testCase: GoldenCase, convId: string): Promise<void> {
  ...
  await prisma.conversationContext.upsert({
    ...
    update: { extractedEntities: { confirmedItems: cart, discussedItems: [], lastAmbiguousPrompt: null } as any },
    create: {
      id: 'ctx-' + convId,
      conversationId: convId,
      lastMessages: [],
      extractedEntities: { confirmedItems: cart, discussedItems: [], lastAmbiguousPrompt: null } as any,
      userIntent: null,
...
async function cleanupDb(testCase: GoldenCase, convId: string): Promise<void> {
  try {
    if (testCase.context.activeOrder) {
...
function makeCtx(testCase: GoldenCase, convId: string): ConversationContextShape {
  const storeId = testCase.context.storeId || 'store-1';
  return {
    storeId,
    customerId: 'golden-tester',
...
describe('Golden Dataset — ' + TOTAL_CASES + ' cases', () => {
  before(async () => {
    ...
    try {
      ...
      adapters.logger.warn('Redis flush failed (non-critical)', { error: (e as Error).message });
    }
  });
...
test(testCase.name + ' — ' + testCase.description, async () => {
      ...
      assert.equal(
        route.kind,
        expectedKind,
        `Route: expected "${expectedKind}" but got "${route.kind}" for input: "${testCase.input}"`
      );
      ...
      try {
        if (testCase.expect.skipResponse) return;
        ...
        switch (route.kind) {
          case 'order_change':
            if (testCase.context.activeOrder) {
              const result = await fallbackService.handleOrderChangeRequest(
                mockCtx as any, testCase.input, testCase.context.activeOrder.orderStatus
              );
          ...
          case 'cart_modify':
            ...
            if (testCase.expect.cartAfter !== undefined) {
              ...
              const remaining = cartItems.filter(ci =>
                !removeItems.some(rm => ci.product.toLowerCase().includes(rm.toLowerCase()))
              );
              // Cart should have items removed
              assert.ok(remaining.length < cartItems.length,
                `Cart should have fewer items after remove. remove=${JSON.stringify(removeItems)}`);
            }
            break;
          ...
          case 'order_status':
          case 'waterfall':
            const wfResult = await fallbackService.getResponse(mockCtx as any, testCase.input, false, customerCity);
        ...
        if (testCase.expect.source && responseSource) {
          assert.equal(
            responseSource as string,
            testCase.expect.source,
            `Source: expected "${testCase.expect.source}" but got "${responseSource}" for input: "${testCase.input}"`
          );
        }
        ...
        if (testCase.expect.mustContain && responseContent) {
          for (const expected of testCase.expect.mustContain) {
            assert.ok(
              ...
              `Response must contain "${expected}" but got: "${responseContent.slice(0, 200)}"`
            );
          }
        }
        ...
        if (testCase.expect.mustNotContain && responseContent) {
          for (const notExpected of testCase.expect.mustNotContain) {
            assert.ok(
              ...
              `Response must NOT contain "${notExpected}" but got: "${responseContent.slice(0, 200)}"`
            );
          }
        }
      } finally {
