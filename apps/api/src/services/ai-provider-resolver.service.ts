...
const CACHE_TTL_MS = 30_000; // mirrors getAiDefaults (ai-config.ts:41)

export class AIProviderResolverService {
  private cache = new Map<string, { providers: AIProvider[]; ts: number }>();
  ...
  constructor(
     ...
     */
    private findMany: (args: FindManyArgs) => Promise<ProviderRow[]> = (args) =>
      prisma.aIProviderConfig.findMany(args),
  ) {}
  ...
  private buildProvider(row: ProviderRow): AIProvider {
    switch (row.format) {
      ...
      default:
        throw new Error(
          `[AIProviderResolver] unknown format '${row.format}' for provider '${row.name}' ` +
