...
import type { PendingClarification, ResolvedAction, ExtractedEntities, InterpreterOutput } from '../domain/types.js';
 ...
 */
export {
  resolvePendingClarification as resolvePendingClarificationPure,
  isAffirmative,
  isNegation,
  parseExplicitChoice,
  normalizeForMatch,
  selectOption,
} from './chat/pendingClarification.js';
...
export async function resolvePendingClarification(
  ...
  if (!ctxRow) return { handled: false };
  ...
  const pendingWithOptions = {
    ...pending,
    options: (pending.options || pending.rawOptions || []).map((opt: any, i: number) =>
  ...
  const result = pureResolve(customerMessage, pendingWithOptions as any);
    ...
    case 'RESOLVED':
      ...
      if (result.cartOps && result.cartOps.length === 0) {
        ...
        if (previousMutation && previousMutation.cartSnapshot) {
          ...
          const updated: ExtractedEntities = { ...entities, previousMutation: null };
          await prisma.conversationContext.update({
            where: { conversationId },
            data: { extractedEntities: updated as any },
          });
        }
      }

      const reply = await renderPostActionReply(conversationId, storeId, pendingWithOptions as any);
      return {
        handled: true,
 ...
 */
async function renderPostActionReply(
  conversationId: string,
  storeId: string,
  pending: { options: any[]; expected_type: string; question: string }
): Promise<string> {
  const ctxRow = await prisma.conversationContext.findUnique({
    where: { conversationId },
  ...
  const productNames = cart.map((i: { product: string }) => i.product);
  const totalQty = cart.reduce(
    (sum: number, i: { qty?: number | string | null }) => sum + (typeof i.qty === 'number' ? i.qty : 1),
    0
  );
 ...
 */
export function buildPendingFromClarification(
  clarification: NonNullable<InterpreterOutput['clarification']>
): Omit<PendingClarification, 'asked_at' | 'retry_count'> {
  return {
