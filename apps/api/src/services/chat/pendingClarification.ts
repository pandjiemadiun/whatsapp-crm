...
export function isNegation(text: string): boolean {
  ...
  const words = norm.split(/\s+/);
  return (
    NEGATION_WORDS.some((w) => words.includes(w)) ||
 ...
 */
function flattenCartOps(pending: PendingClarification): CartOp[] {
  const opts =
    pending.options && pending.options.length > 0
      ? pending.options
      : (pending.rawOptions || []).map((r: string, i: number) => ({
          id: String(i),
          label: r,
        }));
  ...
  for (const opt of opts) {
    const cartOps = (opt as any).cartOps as CartOp[] | undefined;
 ...
 */
export function selectOption(text: string, options: string[]): string[] {
 ...
 */
export function parseExplicitChoice(
  text: string,
  options: string[]
): string | null {
  const t = text.toLowerCase();
