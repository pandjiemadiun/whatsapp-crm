...
import type { InterpreterResultV2 } from '../types-v2.js';
...
function makeStubFallback(source: ResponseSource = ResponseSource.HUMAN) {
  return {
    getResponse: async (_msg: string, _ctx: unknown) => ({
      source,
      content: 'stub',
      confidence: 0.9,
      cost: 0,
...
function makeValidResult(overrides: Partial<InterpreterResultV2> = {}): InterpreterResultV2 {
  return {
    acts: [
      {
        act_id: 'a1',
...
function makeRetryableResult(): InterpreterResultV2 {
...
function makeTerminalResult(): InterpreterResultV2 {
  return makeValidResult({
    confidence: {
      entities: 0.9,
      intent: 0.9,
...
function makePositionalSupersedesResult(): InterpreterResultV2 {
  return {
    acts: [
      {
        act_id: 'a1',
      ...
      },
      {
        ...
        qty_source: 'explicit',
        confidence: 0.9,
        supersedes: 0 as unknown as null, // positional index → akan dikonversi ke 'a1'
      },
    ],
    unmatched_mentions: [],
    topic_switch: false,
...
const mockGenerate = async (
  ...
  llmCalls++;
  lastPrompt = prompt;
  if (mockThrow !== null) throw new Error(mockThrow);
  const content =
    mockResponses.length > 0 ? mockResponses.shift()! : '';
  return {
    content,
    provider: 'groq',
    model: 'test-model',
...
describe('understand — fast path hit (0 LLM) (FASE B4)', () => {
  it('Fast path hit (pending resolved) → outcome=resolved, llmCalls=0', async () => {
    ...
    const result = await understand(
      'iya', ws, CATALOG, [], makeStubFallback()
    );
  ...
  it('Fast path tier hit (fallback non-HUMAN) → outcome=tier, llmCalls=0', async () => {
    ...
    const result = await understand(
      'katalog', ws, CATALOG, [], makeStubFallback(ResponseSource.CATALOG)
    );
...
describe('understand — LLM reasoning + validator (FASE B4)', () => {
  it('LLM valid + validator ok → reasoned, llmCalls=1', async () => {
    ...
    const result = await understand(
      'mau beli beras', ws, CATALOG, [], makeStubFallback()
    );
  ...
  it('Validator reject retryable=true + retry ok → reasoned, llmCalls=2', async () => {
    ...
    const result = await understand(
      'mau beli apel', ws, CATALOG, [], makeStubFallback()
    );
  ...
  it('Validator reject terminal (low confidence) → fallback, llmCalls=1, JANGAN retry', async () => {
    ...
    const result = await understand(
      'mau beli', ws, CATALOG, [], makeStubFallback()
    );
