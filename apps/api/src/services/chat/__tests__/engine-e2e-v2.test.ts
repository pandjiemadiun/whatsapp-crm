...
import type { InterpreterResultV2, ActV2, WorkspaceV2, PendingV2, DraftCartOp } from '../types-v2.js';
...
function makeFreshWorkspace(): WorkspaceV2 {
  return {
    schema_version: '3.2',
 ...
 */
function makeCartResult(
  product: string,
  qty: number,
  qtySource: 'explicit' | 'default',
  opts: {
    confidence?: number;
...
// ─────────────────────────────────────────────────────────────────────────────
// Simulasi engine v2 pipeline (mimiking conversation.service.ts v2 branch)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Hasil satu siklus message processing di engine v2.
 * Memungkinkan assertion terhadap outcome, llmCalls, reply, dan workspace state.
 */
interface V2TurnResult {
  outcome: string;
  llmCalls: number;
  reply: string;
 ...
 */
async function processMessageV2(
  message: string,
  workspace: WorkspaceV2
  ...
  const reasoningOutcome = await understand(
    ...
    workspace,
    CATALOG,
    [], // history kosong untuk keperluan unit-level
    fallbackStub,
    STORE_ID
  );
  ...
  const reasoned =
    reasoningOutcome.outcome === 'reasoned' ? reasoningOutcome : null;
...
describe('Engine v2 — 3-message end-to-end cart building flow', () => {
  it('Msg 1: "Mau beli beras 2" → LLM cart_update Beras qty 2', async () => {
    ...
    const item1 = result.workspace.draft_cart[0] as DraftCartOp;
  ...
  it('Msg 3: "Eh, gajadi cukup gula 1" → LLM supersede (a1=Beras, a2=Gula), planner drop a1', async () => {
    ...
    const supersedeResult: InterpreterResultV2 = {
      acts: [
        {
          act_id: 'act_beras_v2',
          intent: 'cart_update',
