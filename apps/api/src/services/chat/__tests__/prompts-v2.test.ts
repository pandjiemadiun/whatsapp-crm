...
describe('FEW_SHOTS (FASE B1)', () => {
  ...
  it('setiap expected_json adalah JSON valid & punya field inti InterpreterResultV2', () => {
    ...
    for (const fs of FEW_SHOTS as FewShotV2[]) {
      const parsed: unknown = JSON.parse(fs.expected_json);
  ...
  it('case-2 (revisi) punya act kedua dengan supersedes merujuk act pertama', () => {
    ...
    const parsed: unknown = JSON.parse(fs.expected_json);
    const obj = parsed as { acts: Array<{ act_id: string; supersedes: string | null }> };
  ...
  it('case-4 (mismatch) memiliki mismatch_reason, case-5 (ambiguous) ada clarification', () => {
    const c4 = JSON.parse(FEW_SHOTS[3].expected_json) as { quantifier?: { resolution_type: string; mismatch_reason?: string } };
    assert.equal(c4.quantifier?.resolution_type, 'mismatch');
    assert.equal(typeof c4.quantifier?.mismatch_reason, 'string');

    const c5 = JSON.parse(FEW_SHOTS[4].expected_json) as { clarification?: unknown; quantifier?: { resolution_type: string } };
  ...
  it('case-7 (greeting) → acts kosong + reply_draft ramah (TASK 8 regression A)', () => {
    ...
    const parsed = JSON.parse(fs.expected_json) as {
      acts: unknown[];
      reply_draft?: string;
    };
  ...
  it('case-8 (cancel) → intent cancel + entity produk (TASK 8 regression C)', () => {
    ...
    const parsed = JSON.parse(fs.expected_json) as {
      acts: Array<{ intent: string; entities: Array<{ type: string; value: string }> }>;
    };
  ...
  it('PV-P2c-LLM-B 7a: entities[].metadata.variant === draft_cart_ops[].variant (konsisten) untuk 3 few-shot varian', () => {
    ...
    for (const idx of [8, 9, 10]) {
      ...
      const parsed = JSON.parse(fs.expected_json) as {
        acts: Array<{
          act_id: string;
          entities: Array<{ type: string; value: string; metadata?: { variant?: string } }>;
        }>;
        draft_cart_ops: Array<{ action: string; product: string; variant?: string | null }>;
      };

      // lookup: product value (lowercased) → entity
      const entityByProduct = new Map<string, { metadata?: { variant?: string } }>();
      for (const act of parsed.acts) {
        for (const e of act.entities) {
          if (e.type === 'product') entityByProduct.set(e.value.toLowerCase(), e);
        }
      }
      ...
      for (const op of parsed.draft_cart_ops) {
        ...
        assert.equal(
          ent.metadata?.variant ?? null,
          op.variant ?? null,
          `entity.metadata.variant harus SAMA PERSIS dengan draft_cart_ops.variant untuk "${op.product}" (FS idx ${idx}) — jika golden few-shot sendiri tidak konsisten, instruksi prompt untuk LLM nanti kurang jelas`,
        );
      }
    }
  });
