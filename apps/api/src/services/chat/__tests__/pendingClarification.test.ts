...
} from '../pendingClarification.js';
import type { CartOp } from '../../../domain/types.js';

function makeCtx(
  pending: ResolvePendingContext['pending']
): ResolvePendingContext {
...
describe('resolvePending (BAGIAN 2)', () => {
  it('"dua duanya" + pending.ops -> action: EXECUTE', () => {
    const ops: CartOp[] = [
      { type: 'add' as const, product: 'Beras', qty: 1 },
    ];
    const ctx = makeCtx({ ops, retryCount: 0 });
    const r: ResolvePendingResult = resolvePending(ctx, 'dua duanya');
  ...
  it('"semua" + pending.ops -> action: EXECUTE', () => {
    const ops: CartOp[] = [
      { type: 'add' as const, product: 'Gula', qty: 2 },
    ];
    const ctx = makeCtx({ ops, retryCount: 0 });
    const r: ResolvePendingResult = resolvePending(ctx, 'semua');
    assert.equal(r.action, 'EXECUTE');
    assert.equal(r.ops, ops);
  });
  ...
  it('"ga jadi" + pending.snapshot -> action: ROLLBACK', () => {
    ...
    const r: ResolvePendingResult = resolvePending(ctx, 'ga jadi');
  ...
  it('"iya" + pending.ops -> action: EXECUTE (I10: 0 LLM)', () => {
    const ops: CartOp[] = [
      { type: 'add' as const, product: 'Teh', qty: 1 },
    ];
    const ctx = makeCtx({ ops, retryCount: 0 });
    const r: ResolvePendingResult = resolvePending(ctx, 'iya');
    assert.equal(r.action, 'EXECUTE');
  });

  it('"random" + retryCount=0 -> action: RETRY, retryCount jadi 1', () => {
    const ctx = makeCtx({ retryCount: 0 });
    const r: ResolvePendingResult = resolvePending(ctx, NEUTRAL);
