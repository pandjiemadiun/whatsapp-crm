...
export function productNameExact(query: string, name: string): boolean {
  const norm = (s: string) =>
...
export function productNameStrongFuzzy(query: string, name: string): boolean {
  const q = query.trim().toLowerCase();
  const lower = name.toLowerCase().replace(/[^a-z0-9]/g, ' ').trim();
  if (q.length < 2 || lower.length < 2) return false;

  // (b-i) typo of the whole name within the distance threshold.
  if (levenshtein(q, lower) <= PRODUCT_FUZZY_MAX_DISTANCE) return true;
  ...
  if (lower.startsWith(q) || q.startsWith(lower)) return true;
  ...
  if (nameTokens.length > 0 && qTokens.length > 0) {
    if (nameTokens.every((t) => qTokens.includes(t))) return true;
  }
  return false;
}
 ...
 */
export function shouldAnswerSingleProduct(
  query: string,
  name: string,
  resultCount: number
): boolean {
  ...
  if (productNameExact(q, name)) return true;
  // (b) strong fuzzy, ONLY when this is the single candidate product
  if (resultCount === 1 && productNameStrongFuzzy(q, name)) return true;
