...
import { messageQueueService, isDeadEnd, isDeadEndWithContext, isUrgent, type RawMessage, type QueuedMessage } from './message-queue.service.js';
import { CircuitBreakerService } from './circuit-breaker.service.js';
import { presenceSimulatorService, type StorePresenceInfo, type CustomerPresenceInfo } from './presence-simulator.service.js';
...
export class MessageProcessorService {
  private llmCircuitBreaker: CircuitBreakerService;
  ...
  constructor() {
    ...
    messageQueueService.setFlushHandler((msg, sourceMsg) => {
      this.handleFlushed(msg, sourceMsg).catch((err) =>
        adapters.logger.error('Flushed message processing error', err as Error)
      );
    });
  }
  ...
  async processMessage(input: ProcessMessageInput): Promise<ProcessedResult | null> {
    ...
    let lastAiMessage: string | undefined;
...
result = await this.llmCircuitBreaker.wrap(() =>
    ...
    return {
      ...
      source: result.source as 'cache' | 'faq' | 'knowledge' | 'ai' | 'human' | 'dead_end' | 'product' | 'shipping' | 'payment' | 'sop',
  ...
  private async sendWithPresence(
    ...
    const simulation = await presenceSimulatorService.simulateResponse({
      store: { timezone: input.storeTimezone || 'Asia/Jakarta' } as StorePresenceInfo,
      ...
      gateway: gateway
        ? {
            ...
            setPresence: async (phone: string, state: 'composing' | 'paused' | 'none') => {
   ...
   */
  private async markRead(input: ProcessMessageInput): Promise<void> {
    const gateway = this.getGateway(input.gateway);
    if (gateway && typeof gateway.markRead === 'function') {
      try {
        await gateway.markRead!(input.customerPhone, input.deviceId);
      } catch (err) {
        adapters.logger.warn('markRead failed', err as Error);
      }
    }
  }
  ...
  private async sendQrisFollowUp(input: ProcessMessageInput, imageUrl: string): Promise<void> {
    ...
    try {
      if (input.gateway === 'fonnte' && input.token) {
        ...
        const fonnteMediaEnabled = !!(store?.config as any)?.fonnteMediaEnabled;
        ...
        try {
          ...
          adapters.logger.warn('GOWA sendImage failed, falling back to text link', {
            conversationId: input.conversationId,
            error: (gowaErr as Error).message,
          });
      ...
      adapters.logger.warn('QRIS follow-up failed entirely', { conversationId: input.conversationId, error: (err as Error).message });
    }
  }
   ...
   */
  private async smartRetrySend(
    phone: string,
    content: string,
    config: SendMessageConfig,
    gatewayType: 'gowa' | 'fonnte'
  ): Promise<void> {
   ...
   */
  private async notifyHumanTakeover(input: ProcessMessageInput): Promise<void> {
    try {
      const { prisma } = await import('../infrastructure/prisma.js');
      ...
      adapters.logger.warn('Failed to mark human takeover', err as Error);
    }
  }
