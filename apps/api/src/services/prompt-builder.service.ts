...
export class PromptBuilderService {
   ...
   */
  async generateInitialPrompt(storeId: string): Promise<string> {
    ...
    if (store.acceptsTransfer) payments.push('Transfer Bank');
    if (store.acceptsQris) payments.push('QRIS');
    if (store.acceptsCod) payments.push('COD');
    ...
    if (store.shippingMode === 'pickup') {
      ...
      const parts: string[] = [];
      if (store.shippingFlatInCity !== null) parts.push(`Dalam kota Rp ${store.shippingFlatInCity.toLocaleString('id-ID')}`);
      if (store.shippingFlatOutCity !== null) parts.push(`Luar kota Rp ${store.shippingFlatOutCity.toLocaleString('id-ID')}`);
...
Tugas Anda: Menjawab pertanyaan pelanggan dengan ramah, membantu memilih produk, dan memproses pesanan.`;
   ...
   */
  async saveInitialPromptIfMissing(storeId: string): Promise<void> {
    const existing = await prisma.storeSetting.findUnique({
      where: { storeId_key: { storeId, key: 'ai_system_prompt' } },
      select: { value: true },
    });
    if (existing && existing.value) return; // don't overwrite manual edits
  ...
  private formatOperatingHours(operatingHours: unknown): string | null {
    if (!operatingHours || typeof operatingHours !== 'object') return null;

    if (typeof (operatingHours as any).text === 'string' && (operatingHours as any).text.trim()) {
      return (operatingHours as any).text.trim();
    }

    const days = (operatingHours as any).days || operatingHours;
