...
/**
 * Learning Service — Auto-generate FAQ drafts from AI-answered customer questions.
 *
 * Setiap 24 jam, scan conversation_history untuk pertanyaan pelanggan yang
 * ditjawab oleh AI (source='ai', bukan FAQ cache). Kelompokkan pertanyaan
 * serupa via Groq, generate draft FAQ, dan simpan sebagai FAQ dengan
 * isActive=false + category='ai_suggestion' agar owner harus approve manual.
 */
 ...
 */
export async function analyzeAndGenerateFaqDrafts(storeId: string): Promise<number> {
  const now = new Date();
 ...
 */
async function clusterAndDraftFaqs(questions: string[]): Promise<FaqDraft[] | null> {
...
- Output HANYA JSON valid: [{"question":"...","answer":"...","frequency":3}]`;
  ...
  try {
    ...
    const validDrafts = draftsArray.filter(
      (d) => d && typeof d.question === 'string' && typeof d.answer === 'string' && typeof d.frequency === 'number'
    );
