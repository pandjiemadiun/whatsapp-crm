...
export class PresenceSimulatorService {
  ...
  private static readonly TYPING_SPEED_MS_PER_WORD = 100;
  private static readonly NIGHT_MODE_DELAY_MULTIPLIER = 3;
  private static readonly NIGHT_MODE_TYPING_MULTIPLIER = 1.5;
  private static readonly NIGHT_START = 22;
  private static readonly NIGHT_END = 8;
  ...
  getRandomPresenceState(): 'composing' | 'paused' | 'none' {
    const r = Math.random();
    if (r < 0.70) return 'composing';
    if (r < 0.80) return 'paused';
    return 'none';
  }
  ...
  async simulateResponse(opts: {
    ...
    customer?: CustomerPresenceInfo;
    content: string;
    gateway?: { markRead?: (phone: string) => Promise<void>; setPresence?: (phone: string, state: 'composing' | 'paused' | 'none') => Promise<void> };
    phone: string;
    ...
    if (opts.gateway?.setPresence) {
      await opts.gateway.setPresence(opts.phone, presenceState).catch((err) => {
        adapters.logger.warn('setPresence failed', err as Error);
      });
    }
   ...
   */
  private getHoursInTimezone(date: Date, tz: string): number {
  ...
  private isOutsideOperatingHours(hour: number, days: Record<string, { open: string; close: string }>): boolean {
