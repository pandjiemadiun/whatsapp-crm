 ...
 */
export async function runAutoCancelOnce(now: Date = new Date()): Promise<number> {
  ...
  for (const order of orders) {
    try {
      await prisma.$transaction(async (tx) => {
        ...
        await transitionOrder(order.id, 'cancelled', { actor: 'system:auto-cancel', tx: tx as any });
      });
      cancelled++;
 ...
 */
export function scheduleAutoCancel(): void {
  cron.schedule('*/15 * * * *', async () => {
    try {
      ...
      adapters.logger.error('[AutoCancel] sweep errored', error as Error);
    }
  });
