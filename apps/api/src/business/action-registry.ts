...
function cartLinesToResponse(lines: CartLine[]) {
...
export async function handleRemoveFromCart(
  ...
  context: ActionContext
): Promise<ActionResult<RemoveFromCartResponse>> {
  const { actionId, payload } = request;
  const { storeId, customerId, conversationId } = context;
...
export async function handleUpdateCartQuantity(
  ...
  context: ActionContext
): Promise<ActionResult<UpdateCartQuantityResponse>> {
  const { actionId, payload } = request;
  const { storeId, customerId, conversationId } = context;
...
export const actionRegistry: Record<string, ActionDefinition<any, any>> = {
  ...
  REMOVE_FROM_CART: {
    ...
    authorize: async (_request: RemoveFromCartRequest, _context: ActionContext) => {
  ...
  UPDATE_CART_QUANTITY: {
    ...
    authorize: async (_request: UpdateCartQuantityRequest, _context: ActionContext) => {
