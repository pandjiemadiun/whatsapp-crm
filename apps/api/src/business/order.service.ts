...
function validateParsedOrder(raw: any): raw is ParsedOrder {
  if (!raw || typeof raw !== 'object') return false;
  if (!raw.intent || !['buy', 'inquiry'].includes(raw.intent)) return false;
  if (raw.items !== undefined && !Array.isArray(raw.items)) return false;
  return true;
}
...
return parsed;
      ...
      adapters.logger.error('Order extraction failed (non-blocking)', error as Error);
      return null;
    }
  }
