...
import { CANONICAL_SCHEMA_VERSION, DEFAULT_CANONICAL_STATE } from '../canonical-context.service.js';
import {
  loadCanonical,
  saveCanonical,
  ...
  setLastBotMessage,
  getOptionsPresented,
} from '../canonical-context.service.js';
import type { CanonicalConversationState } from '../canonical-context.service.js';
import { CanonicalConversationStateService } from '../canonical-context.service.js';
import type { ExtractedEntities, PendingClarification, ClarificationOption, DiscussedItem, ExtractedEntity, ConfirmedItem } from '../../domain/types.js';
...
function stubPrisma(mock: {
  ...
  if (mock.findUnique) {
    ctx.findUnique = mock.findUnique as PrismaMethod;
  }
  if (mock.updateMany) {
    ctx.updateMany = mock.updateMany as PrismaMethod;
  }
}
...
function makeCanonicalState(overrides: Partial<CanonicalConversationState> = {}): CanonicalConversationState {
  return {
...
describe('loadCanonical / saveCanonical (FASE A2)', () => {
  ...
  it('2. valid canonical state → parsed correctly', () => {
    const state: CanonicalConversationState = makeCanonicalState({
  ...
  it('4c. pendings non-array → defaults to []', () => {
    ...
    } as unknown as CanonicalConversationState);
    assert.deepEqual(state.pendings, []);
  });
  ...
  it('5. unknown fields (draft_cart, confirmedItems) → ignored, NOT stored in canonical state', () => {
    ...
    assert.equal((state as unknown as Record<string, unknown>).draft_cart, undefined);
    assert.equal((state as unknown as Record<string, unknown>).confirmedItems, undefined);
  });
  ...
  it('5b. non-object input (number, array, boolean) → default state', () => {
    ...
    assert.deepEqual(loadCanonical([1, 2, 3] as unknown[]).pendings, []);
  });
...
describe('fromLegacyExtractedEntities (legacy mapping — NO silent field loss)', () => {
  it('6. full legacy extraction mapping → canonical state', () => {
    const entities: ExtractedEntities & Record<string, unknown> = {
  ...
  it('legacy mapping with logger — confirmedItems triggers warning', () => {
    ...
    const cartWarning = logger.warnings.find((w) =>
      w.msg.includes('confirmedItems') && w.msg.includes('cart authority'),
    );
  ...
  it('legacy PendingClarification with rawOptions (string[]) → normalized to PendingV2.options', () => {
    const pc = {
      id: 'q1',
      question: 'Pilih?',
      options: [] as ClarificationOption[],
...
describe('cart boundary — canonical state hanya cart_ref (FASE A2)', () => {
  ...
  it('19b. draft_cart (from V2 workspace) tidak masuk canonical state', () => {
    const state = loadCanonical({
      schema_version: '',
      pendings: [],
      draft_cart: [{ action: 'add' as const, product: 'Ayam', qty: 2, qty_source: 'default', status: 'confirmed' }],
      resolved_facts: {},
      conversation_summary: '',
      options_presented: [],
    });
...
describe('CanonicalConversationStateService — atomic update (FASE A2)', () => {
  ...
  it('15. getCanonical → updateCanonical → persisted state', async () => {
    ...
    stubPrisma({
      ...
      updateMany: async (args: unknown) => {
        const data = (args as { data: { workspace_v2: unknown } }).data;
    ...
    const parsed = loadCanonical(savedState as string);
  ...
  it('16c. TRUE concurrent CAS — two simultaneous updateCanonical, no lost update', async () => {
    ...
    let stored: unknown = saveCanonical(makeCanonicalState({ intent: 'browse' }));
    ...
    const pendingA: PendingV2 = makePending({ id: 'pa', question: 'Q from A?' });
    const pendingB: PendingV2 = makePending({ id: 'pb', question: 'Q from B?' });
  ...
  it('updateResolvedFacts — partial update, preserves existing facts', async () => {
    ...
    stubPrisma({
      ...
      updateMany: async (args: unknown) => {
        savedState = (args as { data: { workspace_v2: unknown } }).data.workspace_v2;
        return { count: 1 };
      },
    });
...
describe('pending clarification boundary (FASE A2)', () => {
  ...
  it('17. question → answer flow: upsertPending → getPendingClarification → resolvePending', async () => {
    const { canonicalConversationStateService } = await import('../canonical-context.service.js');
    let savedWorkspace: unknown = null;
    ...
    stubPrisma({
      ...
      updateMany: async (args: unknown) => {
        savedWorkspace = (args as { data: { workspace_v2: unknown } }).data.workspace_v2;
        return { count: 1 };
      },
    });
  ...
  it('upsertPending replaces existing pending with same ID', async () => {
    ...
    stubPrisma({
      ...
      updateMany: async (args: unknown) => {
        const saved = (args as { data: { workspace_v2: unknown } }).data.workspace_v2;
...
describe('G2-D.2 V1 read↔write split-brain (writeV1* → getV1* round-trip)', () => {
  ...
  it('V1-R4: writeV1PreviousMutation → getV1PreviousMutation preserves cart snapshot and message', async () => {
    ...
    assert.equal((read!.cartSnapshot[0] as { product: string }).product, 'Ayam');
    assert.equal((read!.cartSnapshot[1] as { product: string }).product, 'Sapi');
  });
  ...
  it('V1-R6: full lifecycle — write pending → increment retry → resolve → clear previous mutation', async () => {
    ...
    await canonicalConversationStateService.writeV1PreviousMutation('conv-X',
      [{ product: 'Backup', qty: 1, price: 100 }], 'backup msg');
...
describe('G2-D.3 V2 read → canonical boundary', () => {
  ...
  it('V2-R8: audit — getV2Workspace reads from same canonical column as updateCanonical', async () => {
    ...
    let stored: unknown = saveCanonical(makeCanonicalState({
      intent: 'support',
    ...
    await canonicalConversationStateService.updateCanonical('conv-audit', (state) =>
      setIntent(state, 'support'),
    );
...
describe('G2-D.5 V2 write → canonical', () => {
  let canonicalConversationStateService: Awaited<ReturnType<typeof import('../canonical-context.service.js')>>['canonicalConversationStateService'];
  before(async () => {
  ...
  it('D5-R5: options_presented preserved after saveWorkspaceV2', async () => {
    ...
    await canonicalConversationStateService.saveWorkspaceV2('conv-d5-5', {
     ...
     const opts = getOptionsPresented(
       (await canonicalConversationStateService.getCanonical('conv-d5-5'))!
     );
...
describe('G2-D.6 Compatibility Reader Audit — V1 discussedItems/previousMutation canonical authority', () => {
  let canonicalConversationStateService: CanonicalConversationStateService;
  before(() => {
  ...
  it('D6-R8: V1 write → canonical → V1 read (storePreviousMutation → getV1PreviousMutation)', async () => {
    ...
    assert.ok(read);
    assert.equal(read!.cartSnapshot.length, 1);
    assert.equal((read!.cartSnapshot[0] as any).product, 'Ayam');
  ...
  it('D6-R9: writeV1DiscussedItems preserves other canonical state (pendings, resolved_facts)', async () => {
    ...
    const items: DiscussedItem[] = [{ product: 'Beras', qty: null, price: null, unit: 'unit', mentionedAt: '2026-08-14T00:00:00Z' }];
