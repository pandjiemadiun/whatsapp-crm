...
const PAYMENT_METHODS = ['transfer', 'qris'] as const;
...
export class PaymentService {
  ...
  async reportPayment(
    ...
    if (!order) throw new ApiError(ErrorCodes.ERR_NOT_FOUND, 'Pesanan tidak ditemukan');
    ...
    try {
      ...
      if (e?.code === 'P2025') {
        throw new ApiError(
          ...
          'Pesanan tidak dalam status yang dapat dilaporkan (sudah diverifikasi atau dalam proses)',
        );
      }
      throw e;
    }
  ...
  async verifyPayment(
    ...
    return row as unknown as OrderWithItems;
  }
}
