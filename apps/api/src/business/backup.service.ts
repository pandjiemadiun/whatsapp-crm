...
import { getBackupAdapter, BackupAdapter } from './backup.adapter.js';
...
function execShell(cmd: string, timeout: number) {
  return execAsync(cmd, { timeout, shell: true } as any);
}
...
export class BackupService {
   ...
   */
  private async validateDatabaseTools(): Promise<void> {
    const platform = process.platform;

    if (platform === 'win32') {
      throw new ApiError(
        ErrorCodes.ERR_EXTERNAL_UNAVAILABLE,
        'Database backup requires pg_dump/psql which are not available on Windows. ' +
  ...
  async createDatabaseBackup(type: 'daily' | 'weekly' | 'manual' = 'daily') {
    ...
    try {
      ...
      adapters.logger.error(`[Backup FAILED] ${type} backup`, error as Error);
      ...
      if (backupConfig.notifyOnFailure && backupConfig.notificationEmail) {
        try {
          await sendBackupFailureAlert(
            error as Error,
            `[Backup FAILED] ${type} backup (${filename})`
          );
        } catch (alertError) {
          adapters.logger.error('[Backup Alert] Failed to send alert email', alertError as Error);
  ...
  async getBackupDetail(filename: string) {
    const backup = await prisma.backupManifest.findUnique({ where: { filename } });
    if (!backup || backup.deletedAt) throw new Error('Backup not found');
    return { ...backup, size: Number(backup.size) };
  }

  async verifyBackupIntegrity(filename: string) {
    try {
      ...
      return { valid: false, checksum: '', size: 0, error: (error as Error).message };
    }
  }
  ...
  async restoreDatabase(filename: string) {
    ...
    try {
      ...
      adapters.logger.error(`[RESTORE FAILED]`, error as Error);
      throw error;
    }
  }

  async deleteBackup(filename: string) {
    try { await this.adapter.delete(filename); } catch {}
    await prisma.backupManifest.update({
      where: { filename },
  ...
  async deleteOldBackups() {
    ...
    for (const backup of old) {
      try { await this.adapter.delete(backup.filename); } catch {}
