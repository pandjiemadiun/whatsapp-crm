...
const cache: CachedResult = { data: null, expiresAt: 0 };
...
export class AnalyticsService {
   ...
   */
  async getAnalytics(range: DateRange = '30d', forceRefresh = false): Promise<AnalyticsResult> {
    ...
    const [
      messageVolumeResults,
      costTrendResults,
      responseTimeResults,
      sourceBreakdownResults,
      orderFunnelResults,
      revenueTrendResults,
      activeStoresCount,
      ...
      prisma.$queryRaw<Array<DateBucket>>`
        SELECT 
    ...
    const messageVolumeTrend = messageVolumeResults.map((r) => ({
      date: new Date(r.date as Date).toISOString(),
    ...
    const totalMessagesInPeriod = sourceBreakdownResults.reduce(
    ...
    const totalOrdersInPeriod = orderFunnelResults.reduce(
      (sum, r) => sum + Number(r.count || 0),
      0
    );
