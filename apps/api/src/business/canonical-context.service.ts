...
import type { PendingV2, DraftCartOp, WorkspaceV2 } from '../services/chat/types-v2.js';
import type {
  DiscussedItem,
  ExtractedEntity,
  ConfirmedItem,
...
export interface CanonicalCompatState {
  ...
  previous_mutation: { cart_snapshot: ConfirmedItem[]; message: string } | null;
 ...
 */
export const DEFAULT_CANONICAL_STATE: CanonicalConversationState = {
 ...
 */
export function loadCanonical(raw: unknown): CanonicalConversationState {
  ...
  return {
    schema_version:
      typeof obj.schema_version === 'string' && obj.schema_version.length > 0
        ? obj.schema_version
    ...
    conversation_summary:
      typeof obj.conversation_summary === 'string'
        ...
        : '',
    pendings: Array.isArray(obj.pendings) ? (obj.pendings as PendingV2[]) : [],
    resolved_facts: isPlainObject(obj.resolved_facts)
      ? (obj.resolved_facts as Record<string, unknown>)
      : {},
    intent: typeof obj.intent === 'string' ? obj.intent : null,
    options_presented:
      Array.isArray(obj.options_presented)
        ? (obj.options_presented as string[][])
        : [],
    last_bot_message_type:
      typeof obj.last_bot_message_type === 'string'
...
export function saveCanonical(state: CanonicalConversationState): string {
  return JSON.stringify(state);
}

/** Deserialize cart_ref sub-object. */
function loadCartRef(raw: unknown): CanonicalCartRef {
...
function loadCompat(raw: unknown): CanonicalCompatState | undefined {
  if (raw === null || raw === undefined) return undefined;
  if (isPlainObject(raw)) {
    return {
      discussed_items: isRecordArray(raw.discussed_items)
        ? (raw.discussed_items as unknown as DiscussedItem[])
        : [],
      tracked_entities: isRecordArray(raw.tracked_entities)
        ? (raw.tracked_entities as unknown as ExtractedEntity[])
        : [],
      previous_mutation:
        isPlainObject(raw.previous_mutation)
          ? (raw.previous_mutation as { cart_snapshot: ConfirmedItem[]; message: string })
          : null,
      customer_name:
      ...
      pending_clarification: isPlainObject(raw.pending_clarification)
        ? (raw.pending_clarification as unknown as PendingClarification)
        : null,
    };
  }
  return undefined;
 ...
 */
function normalizePendingClarification(pc: PendingClarification): PendingV2 {
  ...
  if (Array.isArray(rawOptions)) {
    ...
    options = pc.options
      .map((o: ClarificationOption | string): string =>
        typeof o === 'string'
          ? o
          : o.label ?? o.id ?? '',
      )
  ...
  const id =
    typeof (pc as unknown as { id?: unknown }).id === 'string' &&
    (pc as unknown as { id: string }).id.length > 0
      ? (pc as unknown as { id: string }).id
  ...
  return {
    ...
    status: 'active',
    attempts:
      typeof pc.retry_count === 'number' ? pc.retry_count : 0,
    deferred_turns: 0,
    asked_at:
      typeof pc.asked_at === 'string' && pc.asked_at.length > 0
        ? pc.asked_at
 ...
 */
function extractStringField(obj: Record<string, unknown>, key: string): string | null {
  const val = obj[key];
 ...
 */
export function fromLegacyExtractedEntities(
  raw: unknown,
  ...
  for (const field of RESOLVED_FACT_FIELDS) {
    ...
    if (val !== null) resolved_facts[field] = val;
  }
  ...
  if (isPlainObject(pc)) {
    pendings.push(normalizePendingClarification(pc as unknown as PendingClarification));
  }
  ...
  const pending_clarification = isPlainObject(entities.pendingClarification)
    ? (entities.pendingClarification as unknown as PendingClarification)
    : null;

  // Konversi legacy previousMutation (camelCase cartSnapshot) → canonical snake_case
  const legacyPreviousMutation = entities.previousMutation as
    | { cartSnapshot?: unknown; message?: unknown }
    | null
    | undefined;
  const previous_mutation =
    isPlainObject(legacyPreviousMutation)
      ? {
          cart_snapshot: Array.isArray(legacyPreviousMutation.cartSnapshot)
            ? (legacyPreviousMutation.cartSnapshot as unknown as ConfirmedItem[])
            : [],
          message:
            typeof legacyPreviousMutation.message === 'string'
  ...
  const hasCompat =
    Array.isArray(entities.discussedItems) ||
  ...
  const _compat: CanonicalCompatState | undefined = hasCompat
    ? {
        discussed_items: Array.isArray(entities.discussedItems)
          ? (entities.discussedItems as unknown as DiscussedItem[])
          : [],
        tracked_entities: Array.isArray(entities.trackedEntities)
          ? (entities.trackedEntities as unknown as ExtractedEntity[])
          : [],
        previous_mutation,
        customer_name,
        customer_phone,
...
export function findPending(state: CanonicalConversationState, id: string): PendingV2 | undefined {
...
export function getPendings(
  ...
  if (status) return state.pendings.filter((p) => p.status === status);
  return state.pendings;
}

/** Parkirkan (store) pending baru ke state. Mutasi in-place; return state untuk chaining. */
export function parkPending(state: CanonicalConversationState, pending: PendingV2): CanonicalConversationState {
  state.pendings.push(pending);
  return state;
}

/** Aktifkan kembali pending 'deferred' → 'active'. */
export function resumePending(state: CanonicalConversationState, id: string): PendingV2 | undefined {
...
export function resolvePending(state: CanonicalConversationState, id: string): PendingV2 | undefined {
  const p = findPending(state, id);
...
export function dropPending(state: CanonicalConversationState, id: string): PendingV2 | undefined {
  const p = findPending(state, id);
...
export function incrementAttempts(state: CanonicalConversationState, id: string): PendingV2 | undefined {
  const p = findPending(state, id);
  if (p) p.attempts += 1;
  return p;
}
...
export function incrementDeferredTurns(state: CanonicalConversationState, id: string): PendingV2 | undefined {
  const p = findPending(state, id);
...
export function getActivePending(state: CanonicalConversationState): PendingV2 | undefined {
...
export function getFact(state: CanonicalConversationState, key: string): unknown {
...
export function setIntent(state: CanonicalConversationState, intent: string | null): CanonicalConversationState {
  state.intent = intent;
  return state;
}

/** Get conversation intent. */
export function getIntent(state: CanonicalConversationState): string | null {
  return state.intent;
}

/** Set conversation summary. Mutasi in-place; return state untuk chaining. */
export function setSummary(state: CanonicalConversationState, summary: string): CanonicalConversationState {
  state.conversation_summary = summary;
  return state;
}

/** Get conversation summary. */
export function getSummary(state: CanonicalConversationState): string {
  return state.conversation_summary;
}

/** Set cart reference (order_id). Mutasi in-place; return state untuk chaining. */
export function setCartRef(state: CanonicalConversationState, orderId: string | null): CanonicalConversationState {
  state.cart_ref.order_id = orderId;
  return state;
}

/** Get cart reference. */
export function getCartRef(state: CanonicalConversationState): CanonicalCartRef {
...
export function setLastBotMessage(
  ...
  if (options.length > 0) state.options_presented.push(options);
  return state;
}

/** Get options presented history. */
export function getOptionsPresented(state: CanonicalConversationState): string[][] {
...
export class CanonicalConversationStateService {
  ...
  private async atomicCas<T>(
    ...
    operation: string,
    writer: (row: { workspace_v2: unknown; updatedAt: Date }) => Promise<{ count: number | null; value: T }>,
  ): Promise<T | null> {
    for (let attempt = 0; attempt <= ATOMIC_MAX_ATTEMPTS; attempt++) {
      let row: { workspace_v2: unknown; updatedAt: Date } | null;
      try {
        ...
        adapters.logger.error('canonical atomicCas read failed', error as Error, { conversationId, operation });
        return null;
      }
      if (!row) {
      ...
      try {
        ...
        adapters.logger.error('canonical atomicCas write failed', error as Error, { conversationId, operation });
        return null;
      }
   ...
   */
  async getCanonical(conversationId: string): Promise<CanonicalConversationState | null> {
    ...
    */
   async getPendingClarification(conversationId: string): Promise<PendingV2 | undefined> {
    ...
    * Baca resolved fact.
    */
   async getFact(conversationId: string, key: string): Promise<unknown> {
     const state = await this.getCanonical(conversationId);
     if (!state) return undefined;
    ...
    */
   async getV1PendingClarification(conversationId: string): Promise<PendingClarification | null> {
    ...
    */
    async getFactWithLegacyFallback(conversationId: string, key: string): Promise<unknown> {
      const state = await this.getCanonicalWithLegacyFallback(conversationId);
      if (!state) return undefined;
      return getFact(state, key);
    }
     ...
     */
    async getV2Workspace(conversationId: string): Promise<WorkspaceV2 | null> {
      ...
      try {
        ...
        if (row?.workspace_v2 !== null && row?.workspace_v2 !== undefined && row.workspace_v2 !== '') {
          const raw = row.workspace_v2 as Record<string, unknown>;
          if (Array.isArray(raw.draft_cart)) {
            draftCart = raw.draft_cart as DraftCartOp[];
          }
        }
      } catch (err) {
    ...
    */
    async getV1PreviousMutation(conversationId: string): Promise<{ cartSnapshot: unknown[]; message: string } | null> {
      const state = await this.getCanonicalWithLegacyFallback(conversationId);
      if (!state?._compat?.previous_mutation) return null;
     ...
     */
    async getV1TrackedEntities(conversationId: string): Promise<ExtractedEntity[]> {
      const state = await this.getCanonicalWithLegacyFallback(conversationId);
      if (!state?._compat?.tracked_entities) return [];
     ...
     * READ-ONLY.
     */
    async getV1DiscussedItems(conversationId: string): Promise<DiscussedItem[]> {
      const state = await this.getCanonicalWithLegacyFallback(conversationId);
      if (!state?._compat?.discussed_items) return [];
  ...
  async updateCanonical(
    conversationId: string,
    updater: (state: CanonicalConversationState) => CanonicalConversationState,
  ): Promise<CanonicalConversationState | null> {
    return this.atomicCas(conversationId, 'updateCanonical', async (row) => {
      const state = loadCanonical(row.workspace_v2);
      const updated = updater(state);
      const json = updated as unknown as Prisma.InputJsonValue;
   ...
   */
  async updateIntent(conversationId: string, intent: string | null): Promise<CanonicalConversationState | null> {
   ...
   */
  async setCartRef(conversationId: string, orderId: string | null): Promise<CanonicalConversationState | null> {
  ...
  async recordBotMessage(
    ...
    return this.updateCanonical(conversationId, (state) =>
      setLastBotMessage(state, type, options),
    );
  }
   ...
   */
  async upsertPending(conversationId: string, pending: PendingV2): Promise<CanonicalConversationState | null> {
    return this.updateCanonical(conversationId, (state) => {
   ...
   */
  async resolvePending(conversationId: string, id: string): Promise<CanonicalConversationState | null> {
    return this.updateCanonical(conversationId, (state) => {
   ...
   */
  async dropPending(conversationId: string, id: string): Promise<CanonicalConversationState | null> {
    return this.updateCanonical(conversationId, (state) => {
   ...
   */
  async clearAllPending(conversationId: string): Promise<CanonicalConversationState | null> {
    return this.updateCanonical(conversationId, (state) => {
      state.pendings = [];
      return state;
    });
   ...
   */
  async writeV1PendingClarification(
    conversationId: string,
   ...
   */
  async clearV1PendingClarification(conversationId: string): Promise<void> {
    await this.updateCanonical(conversationId, (state) => {
      state.pendings = [];
      if (state._compat) {
   ...
   */
  async incrementV1PendingRetry(conversationId: string): Promise<void> {
    await this.updateCanonical(conversationId, (state) => {
      state.pendings = state.pendings.map((p) =>
        p.status === 'active' ? { ...p, attempts: p.attempts + 1 } : p,
      );
      if (state._compat?.pending_clarification) {
  ...
  async writeV1PreviousMutation(
    ...
    await this.updateCanonical(conversationId, (state) => {
      ...
      state._compat.previous_mutation = {
        cart_snapshot: cartSnapshot as unknown as ConfirmedItem[],
        message,
      };
      return state;
    });
   ...
   */
   async clearV1PreviousMutation(conversationId: string): Promise<void> {
     await this.updateCanonical(conversationId, (state) => {
       if (state._compat) {
    ...
    */
   async writeV1ShippingInfo(
     conversationId: string,
     recipientName?: string | null,
     shippingAddress?: string | null,
   ): Promise<void> {
     const facts: Record<string, unknown> = {};
     if (recipientName !== undefined) facts.recipientName = recipientName;
     if (shippingAddress !== undefined) facts.shippingAddress = shippingAddress;
    ...
    */
   async getV1ExtractedEntities(conversationId: string): Promise<{
     ...
     return {
       ...
       lastAmbiguousPrompt:
         typeof state.resolved_facts.lastAmbiguousPrompt === 'string'
           ? state.resolved_facts.lastAmbiguousPrompt
           : null,
       recipientName:
         typeof state.resolved_facts.recipientName === 'string'
           ? state.resolved_facts.recipientName
           : null,
       shippingAddress:
         typeof state.resolved_facts.shippingAddress === 'string'
       ...
       previousMutation: compat?.previous_mutation
         ? {
             cartSnapshot: (compat.previous_mutation.cart_snapshot ?? []) as unknown as ConfirmedItem[],
   ...
   */
   async saveWorkspaceV2(conversationId: string, workspace: WorkspaceV2): Promise<void> {
     ...
     await this.updateCanonical(conversationId, (state) => {
       ...
       if (workspace.schema_version) state.schema_version = workspace.schema_version;
       if (workspace.last_bot_message_type) state.last_bot_message_type = workspace.last_bot_message_type;
   ...
   */
   private async updateV2Transient(
      conversationId: string,
      transient: { draft_cart?: DraftCartOp[] },
    ): Promise<void> {
      await this.atomicCas(conversationId, 'updateV2Transient', async (row) => {
        ...
        if (row.workspace_v2 && typeof row.workspace_v2 === 'object' && !Array.isArray(row.workspace_v2)) {
          raw = { ...(row.workspace_v2 as Record<string, unknown>) };
        }
        ...
        const result = await prisma.conversationContext.updateMany({
          where: { conversationId, updatedAt: row.updatedAt },
          data: { workspace_v2: raw as unknown as Prisma.InputJsonValue },
        });
        return { count: result.count, value: null as null };
      });
    }
   ...
   */
  async resetCanonical(conversationId: string): Promise<boolean> {
   ...
   */
  async clearFact(conversationId: string, key: string): Promise<CanonicalConversationState | null> {
    return this.updateCanonical(conversationId, (state) => {
   ...
   * Clear intent.
   */
  async clearIntent(conversationId: string): Promise<CanonicalConversationState | null> {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  //
  // Legacy migration (fromLegacyExtractedEntities) adalah pure function yang
  // sudah teruji di atas. DB-level migration (membaca kolom extractedEntities,
  // menulis ke workspace_v2) akan diimplementasikan pada fase G2-D.2+.
  // Service ini tidak melakukan migration DB — hanya menyediakan boundary
  // read/write ke workspace_v2 yang kanonik.
}
