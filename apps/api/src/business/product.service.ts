...
export class ProductService {
   ...
   */
  async getCategoriesByStore(storeId: string): Promise<ProductCategory[]> {
    try {
      ...
      adapters.logger.error('Failed to fetch categories', error as Error, { storeId });
   ...
   */
  async listActiveProducts(storeId: string): Promise<Product[]> {
    try {
      ...
      adapters.logger.error('Failed to list active products', error as Error, { storeId });
  ...
  async getProductsByCategory(categoryId: string): Promise<Product[]> {
    try {
      ...
      adapters.logger.error('Failed to fetch products by category', error as Error, { categoryId });
  ...
  async searchProducts(storeId: string, query: string): Promise<Product[]> {
    ...
    try {
      ...
      adapters.logger.error('Failed to search products', error as Error, { storeId, query });
  ...
  async getProductById(productId: string): Promise<Product> {
    ...
    const mapped = this.mapProduct(row as any);
    // Lampirkan kategori pada detail (optional)
    if ((row as any).category) {
      (mapped as any).category = this.mapCategory((row as any).category);
    }
    return mapped;
  }
  ...
  async createProduct(
    ...
    try {
      ...
      adapters.logger.error('Failed to create product', error as Error, { storeId });
   ...
   */
  async updateProduct(productId: string, data: Partial<Omit<Product, 'id' | 'storeId' | 'createdAt' | 'updatedAt'>>): Promise<Product> {
    ...
    try {
      ...
      adapters.logger.error('Failed to update product', error as Error, { productId });
   ...
   */
  async listProductsByStore(storeId: string, filter?: { categoryId?: string }): Promise<Product[]> {
    try {
      const rows = await prisma.product.findMany({
        where: {
          storeId,
      ...
      adapters.logger.error('Failed to list products', error as Error, { storeId });
  ...
  async getProductsByStore(
    storeId: string,
    options: { limit?: number; offset?: number; sortBy?: 'name' | 'price' | 'createdAt'; order?: 'asc' | 'desc' } = {}
    ...
    const orderBy: Record<string, 'asc' | 'desc'> = { [sortBy]: order };
    ...
    try {
      ...
      return { products: rows.map((r: any) => this.mapProduct(r)), total };
    } catch (error) {
      adapters.logger.error('Failed to fetch products by store', error as Error, { storeId });
  ...
  private mapCategory(raw: any): ProductCategory {
    return {
      id: raw.id,
      storeId: raw.storeId,
      name: raw.name,
  ...
  async magicPaste(
    ...
    if (options.overrides) {
      ...
      if (ov.name !== undefined) raw.name = ov.name;
      if (ov.price !== undefined) raw.price = ov.price;
      if (ov.stock !== undefined) raw.stock = ov.stock;
      if (ov.weight !== undefined) raw.weight = ov.weight;
    ...
    if (raw.error || !raw.name || raw.price == null) {
      ...
      if (raw.price == null) missing.push('price');
   ...
   */
  async magicPasteBatch(
    storeId: string,
    text: string,
    options: { preview?: boolean; source?: 'store' | 'admin' } = {}
  ): Promise<{
    items: Array<{
      index: number;
      line: string;
   ...
   */
private async extractWithLLM(text: string, storeId?: string): Promise<MagicPasteExtraction> {
    ...
    try {
      ...
      const cleaned = result.content
        ...
        .replace(/`/g, '')
        .trim();
      const parsed = JSON.parse(cleaned) as MagicPasteExtraction;
   ...
   */
  async loadPatterns(): Promise<MagicPastePattern[]> {
    try {
      const setting = await prisma.systemSetting.findUnique({
        where: { key: 'magic_paste_patterns' },
      });
      if (!setting) return this.defaultPatterns();
      return JSON.parse(setting.value) as MagicPastePattern[];
    } catch {
   ...
   */
  async savePatterns(patterns: MagicPastePattern[]): Promise<void> {
   ...
   */
  async loadSettings(): Promise<MagicPasteSettings> {
    try {
      const setting = await prisma.systemSetting.findUnique({
        where: { key: 'magic_paste_settings' },
      });
      if (!setting) return this.defaultSettings();
      return { ...this.defaultSettings(), ...(JSON.parse(setting.value) as Partial<MagicPasteSettings>) };
    } catch {
   ...
   */
  async saveSettings(settings: MagicPasteSettings): Promise<void> {
  ...
  private defaultPatterns(): MagicPastePattern[] {
    return [
      {
        id: 'name_price_stock',
  ...
  /** Default settings. */
  private defaultSettings(): MagicPasteSettings {
    return {
   ...
   */
  private async tryPatternExtraction(text: string, settings: MagicPasteSettings): Promise<MagicPasteExtraction | null> {
    ...
    for (const pattern of active) {
      try {
        ...
        if (result.name && result.price != null) {
          return result as MagicPasteExtraction;
        }
      } catch (error) {
   ...
   */
  private regexFallbackExtraction(text: string): MagicPasteExtraction {
    const normalized = text.trim();

    // Hapus pola kuantitas (mis. "1kg", "500gr", "100 ikat") agar tidak
    // salah terbaca sebagai harga pada regex fallback.
    const withoutQuantities = normalized.replace(
      /\b\d+(?:[.,]\d+)?\s*(?:kg|gr|gram|grams|g|pcs|ikat|dus|botol|kaleng|bungkus|sachet|kemasan|helai|biji|pack|lusin|kodi)\b/gi,
      ''
    );
    ...
    const priceMatch = withoutQuantities.match(
      /(?:rp\s*|idr\s*)?(\d+(?:[.,]\d+)*)(?:(?:rb|ribu|juta|jt|[km])(?!\w)|\s+(?:rb|ribu|juta|jt)\b)?/i
    );
    ...
    let stock: number | null = stockMatch ? parseInt(stockMatch[1], 10) : null;
   ...
   */
  private normalizePriceText(input: string): number | null {
   ...
   */
  private extractWeightGrams(text: string): number | null {
    if (!text) return null;
   ...
   * Threshold: >= 0.75.
   */
  private async fuzzyMatchCategory(
    storeId: string,
    categoryNameText: string
  ): Promise<{ categoryId: string; score: number; matchedName: string } | null> {
    ...
    for (const cat of categories) {
      ...
      else if (name.includes(target) || target.includes(name)) score = 0.85;
      else {
      ...
      if (!best || score > best.score) best = { id: cat.id, name: cat.name, score };
    }
   ...
   */
  private async generateUniqueSKU(storeId: string): Promise<string> {
    ...
    for (let attempt = 1; attempt <= ProductService.SKU_MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        adapters.logger.error('SKU check failed', error as Error, { sku });
      }
    }
   ...
   */
  private async recordMagicPasteRun(input: {
    storeId: string;
    ...
    try {
      await prisma.magicPasteRun.create({
        data: {
          ...
          extractedEntities: input.extractedEntities as unknown as Prisma.InputJsonValue,
          source: input.source,
          errorMessage: input.errorMessage ?? null,
        },
      });
    } catch (error) {
      adapters.logger.error('Failed to record magic paste run', error as Error, {
        storeId: input.storeId,
        status: input.status,
      });
    }
...
/** Hasil extraction LLM Magic Paste */
interface MagicPasteExtraction {
  name: string | null;
  price: number | null;
  stock: number | null;
  ...
  priceDisplay?: string;
  /** Berat dalam gram. undefined/null = TIDAK disebut di teks sumber (jangan nebak). */
  weight?: number | null;
