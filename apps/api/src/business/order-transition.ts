...
import type { OrderWithItems, OrderItem } from '../domain/types.js';
...
export class InvalidOrderTransitionError extends Error {
  constructor(
    ...
    super(
      `Invalid order transition: ${fromStatus} → ${toStatus} (orderId=${orderId})`,
    );
...
export async function transitionOrder(
  ...
  if (fromStatus === toStatus) {
    ...
    return mapOrderWithItems(existing as any);
  }
  ...
  return mapOrderWithItems(row as any);
}
...
function mapOrderWithItems(raw: any): OrderWithItems {
  return {
    id: raw.id,
    storeId: raw.storeId,
