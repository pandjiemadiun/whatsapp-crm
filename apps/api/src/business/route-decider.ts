...
import { ConversationContext, ResponseSource } from '../domain/types.js';
...
export interface RouteContext {
  ...
  lowerMsg: string;
  previousMutation?: { cartSnapshot: ConfirmedItemLike[]; message: string; } | null;
}
...
export async function buildRouteContext(
  ...
  const cart = Array.isArray(raw.confirmedItems) ? (raw.confirmedItems as ConfirmedItemLike[]) : [];
  const previousMutation = raw.previousMutation as { cartSnapshot: ConfirmedItemLike[]; message: string; } | null || null;
  ...
  return {
    ...
    storeId,
    cart,
    activeOrder: activeOrder as ActiveOrderLike | null,
    customerCity,
    lowerMsg,
    previousMutation,
  };
 ...
 */
export async function decideRoute(ctx: RouteContext): Promise<RouteDecision> {
 ...
 */
async function contextInterpreter(ctx: RouteContext): Promise<RouteDecision | null> {
  ...
  const orderItems = activeOrder && Array.isArray(activeOrder.items)
    ? activeOrder.items.map((i: any) => i.product || i.productName || 'produk').join(', ')
    : '-';
  ...
  try {
    ...
    const parsed = JSON.parse(result.content) as {
      intent: string;
      cart_ops?: { add?: string[]; remove?: string[] };
      confidence?: number;
    };
    ...
    if (parsed.intent === 'CART_MODIFY') {
      ...
      const addItems = (parsed.cart_ops?.add || []).filter(a =>
        productList.some(p => p.toLowerCase().includes(a.toLowerCase().split(' ').pop() || ''))
      );
