...
async getResponse(
  ...
  private async saveDiscussedItems(
    ...
    try {
      ...
      const saved = await conversationContextService.atomicCasExtractedEntities<{
        items: DiscussedItem[];
        lastAmbiguous: string | null;
      }>(
        conversationId,
        'saveDiscussedItems',
        async (row) => {
          ...
          const mergedDiscussedItems: DiscussedItem[] = [...existingDiscussed, ...dedupedNew].slice(-10);
  ...
  private async writeCanonicalDiscussed(
    conversationId: string,
    items: DiscussedItem[],
    lastAmbiguous: string | null
  ): Promise<void> {
