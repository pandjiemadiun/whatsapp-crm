...
export class KeyRotationService {
  ...
  async dryRun(): Promise<DryRunResult> {
    ...
    for (const { model, table, fields, hasSoftDelete } of ENCRYPTED_MODELS) {
      const modelDry: ModelDryRun = { rowCount: 0, encryptedFieldCount: 0, fields: {} };
      ...
      `;
      const countRes = await prisma.$queryRawUnsafe<Array<{ count: number }>>(sql);
      ...
      for (const field of fields) {
        ...
        `;
        const encRes = await prisma.$queryRawUnsafe<Array<{ count: number }>>(encSql);
   ...
   */
  async rotate(newKeyRaw: string): Promise<RotationResult> {
    ...
    if (!currentKeyRaw) currentKeyRaw = process.env.FIELD_ENCRYPTION_KEY || null;
    if (!currentKeyRaw) {
    ...
    await prisma.$transaction(async (tx) => {
      for (const { model, table, fields, hasSoftDelete } of ENCRYPTED_MODELS) {
        ...
        const selectSql = `
          ...
          WHERE (${nullCondition})${deletedClause}
        `;
        const rows = await tx.$queryRawUnsafe<Array<Record<string, string | null & { id: string }>>>(selectSql);
        ...
        for (const row of rows) {
          ...
          await tx.$executeRawUnsafe(
            `UPDATE "${table}" SET ${paramPlaceholders} WHERE "id"::text = ${idParam}`,
            ...paramValues, row.id
          );
