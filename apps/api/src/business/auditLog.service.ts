...
export async function searchLogs(filters: AuditLogSearchFilters = {}) {
...
export async function getLogStats(storeId?: string) {
  const where: Record<string, unknown> = {};
  ...
  for (const log of logs) {
    ...
    if (log.userId) userCount[log.userId] = (userCount[log.userId] || 0) + 1;
  }
...
export async function exportLogs(filters: ExportFilters, format: 'json' | 'csv'): Promise<string> {
  const where: Record<string, unknown> = {};
