...
describe('GeminiAdapter (existing production adapter — HTTP mocked, no real Gemini calls)', () => {
  it('case1: successful generate() — body {contents/parts/generationConfig} + parsed content + usage/cost', async () => {
    calls.length = 0;
    responder = async () =>
      contentResp('Hai Kak! Ada yang bisa saya bantu?', {
        usageMetadata: {
          promptTokenCount: 10,
    ...
    assert.ok(Math.abs(r.cost - ((10 * 0.075 + 10 * 0.3) / 1_000_000)) < 1e-12);
  });
  ...
  it('case3c: 500 → AIProviderError SERVER_ERROR, retryable', async () => {
    calls.length = 0;
    responder = async () =>
      fakeResponse({ ok: false, status: 500, statusText: 'Server Error', text: 'boom' });
    const adapter = new GeminiAdapter('k');
    await assert.rejects(
      adapter.generate('hi'),
      (e) =>
