...
function contentResp(text: string, extra: Record<string, unknown> = {}): FakeResp {
  return fakeResponse({
    ok: true,
...
describe('GeminiShimAdapter (NEW parameterized Gemini shim — fully hermetic)', () => {
  it('case1: success — native body {contents/parts/generationConfig} + URL ?key= + parsed content + thoughts-aware tokens + cost', async () => {
    ...
    assert.ok(Math.abs(r.cost - ((12 * 0.075 + 12 * 0.3) / 1_000_000)) < 1e-12);
  });
  ...
  it('case4: malformed (parts[0].text missing) → UNKNOWN AIProviderError', async () => {
    calls.length = 0;
    responder = async () =>
      fakeResponse({ ok: true, json: { candidates: [{ content: { parts: [{}] } }] } });
    await assert.rejects(
      adapter().generate('hi'),
      (e) => e instanceof AIProviderError && e.category === ErrorCategory.UNKNOWN,
    );
  });
  ...
  it('case7: 429 → RATE_LIMIT, retryable, Retry-After parsed', async () => {
    calls.length = 0;
    responder = async () =>
      fakeResponse({ ok: false, status: 429, statusText: 'Too Many Requests', headers: { 'Retry-After': '9' }, text: '{}' });
    await assert.rejects(
      adapter().generate('hi'),
      (e) =>
        e instanceof AIProviderError &&
