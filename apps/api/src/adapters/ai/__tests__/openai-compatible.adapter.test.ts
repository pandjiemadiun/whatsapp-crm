...
const calls: Call[] = [];
let responder: (url: string, body: unknown) => Promise<FakeResp> = async () =>
  fakeResponse({ ok: true, json: { choices: [{ message: { content: 'x' } }] } });
...
before(() => {
  ...
  console.warn = () => {};
  console.error = () => {};
  global.fetch = ((input: URL | string, init?: RequestInit) => {
    const url = typeof input === 'string' ? input : String(input);
    const body = init?.body ? JSON.parse(String(init.body)) : undefined;
    calls.push({ url, init, parsedBody: body });
    return Promise.resolve(responder(url, body) as unknown as Response);
...
describe('OpenAICompatibleAdapter (NEW generic adapter — fully hermetic)', () => {
  it('case1: success — OpenAI body shape + Bearer auth + parsed content + tokens + cost', async () => {
    ...
    assert.equal((calls[0].init as any).headers.Authorization, 'Bearer sk-test-abc');
    ...
    assert.ok(Math.abs(r.cost - ((8 * 0.05 + 4 * 0.15) / 1_000_000)) < 1e-12);
  });

  it('case2: jsonMode adds response_format: { type: json_object }', async () => {
    calls.length = 0;
    responder = async () =>
      fakeResponse({ ok: true, json: { choices: [{ message: { content: '{}' } }] } });
    const adapter = new OpenAICompatibleAdapter({ baseUrl: BASE_URL, apiKey: 'k', model: 'm' });
    await adapter.generate('hi', { jsonMode: true });
    assert.deepEqual((calls[0].parsedBody as any).response_format, { type: 'json_object' });
  });
  ...
  it('case2b: without jsonMode, response_format is absent', async () => {
    ...
    assert.equal((calls[0].parsedBody as any).response_format, undefined);
  });

  it('case3: usage fallback (no usage) → ceil(prompt/content length / 4)', async () => {
    calls.length = 0;
    responder = async () =>
      fakeResponse({ ok: true, json: { choices: [{ message: { content: 'Hello!' } }] } });
    const adapter = new OpenAICompatibleAdapter({ baseUrl: BASE_URL, apiKey: 'k', model: 'm' });
  ...
  it('case4: 401 → AUTH_ERROR, statusCode 401, not retryable', async () => {
    calls.length = 0;
    responder = async () =>
      fakeResponse({ ok: false, status: 401, statusText: 'Unauthorized', text: '{"error":{"message":"no"}}' });
    const adapter = new OpenAICompatibleAdapter({ baseUrl: BASE_URL, apiKey: 'k', model: 'm' });
  ...
  it('case5: 429 → RATE_LIMIT, retryable, Retry-After parsed', async () => {
    calls.length = 0;
    responder = async () =>
      fakeResponse({ ok: false, status: 429, statusText: 'Too Many Requests', headers: { 'Retry-After': '7' }, text: '{}' });
    const adapter = new OpenAICompatibleAdapter({ baseUrl: BASE_URL, apiKey: 'k', model: 'm' });
    await assert.rejects(
      adapter.generate('hi'),
      (e) =>
  ...
  it('case9: isHealthy() — 2xx→true, non-2xx→false, throw→false', async () => {
    ...
    assert.equal((calls[0].init as any).method, 'GET');
    assert.equal((calls[0].init as any).headers.Authorization, 'Bearer sk-test-abc');
