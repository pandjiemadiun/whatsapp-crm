...
export class GeminiAdapter implements AIProvider {
  ...
  private _model: string = 'gemini-2.0-flash';
...
generationConfig: {
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${this._model}:generateContent?key=${this.apiKey}`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        ...
        const err = new AIProviderError(
          ...
          'gemini',
          response.status,
          this.isRetryable(errorCategory),
          retryAfter,
        );
        throw err;
      }
      ...
      // ========================================
      const inputCost = (promptTokenCount / 1_000_000) * 0.075;
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        ...
        throw new AIProviderError(
          `Gemini request timeout after ${REQUEST_TIMEOUT_MS}ms`,
          ErrorCategory.NETWORK_TIMEOUT,
          'gemini',
          undefined,
          true // timeout adalah retryable
        );
      }
