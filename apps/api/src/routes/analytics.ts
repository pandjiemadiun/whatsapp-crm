...
//   offset?  (default 0)
router.get('/magic-paste', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const storeId = req.user!.storeId;

    const where: Record<string, unknown> = { storeId };
    if (req.query.from) {
      where.createdAt = { ...(where.createdAt as object), gte: parseLocalDay(String(req.query.from), false) };
    }
    if (req.query.to) {
      where.createdAt = { ...(where.createdAt as object), lte: parseLocalDay(String(req.query.to), true) };
    }
    ...
    const trendMap = new Map<string, { date: string; count: number; avgConfidence: number; sumConfidence: number; failed: number }>();
    ...
    res.json({
      ...
      data: {
        ...
        history: recentRuns.map((r) => ({
          ...
          extractedName: (r.extractedEntities as any)?.name ?? null,
          categoryHint: (r.extractedEntities as any)?.categoryHint ?? null,
          source: r.source,
    ...
    adapters.logger.error('Magic paste analytics failed', error as Error);
