...
import type { ProductImage } from '../../domain/types.js';
 ...
 */
router.post(
  '/products/magic-paste',
  validateRequest(magicPasteSchema, 'body'),
  async (req: AuthenticatedAdminRequest, res: Response) => {
    try {
      const { storeId, text, variantOverrides } = getValidated<MagicPasteInput>(req);
      const preview = req.query.preview === 'true';

      const result = await productService.magicPaste(storeId, text, {
        preview,
        source: 'admin',
        variantOverrides: variantOverrides as any,
      });
      ...
      if (error instanceof ApiError) {
        ...
        if (error.details) payload.error = { ...(payload.error as object), details: error.details };
        return res.status(error.statusCode).json(payload);
      }
      adapters.logger.error('Magic paste failed', error as Error);
      res.status(500).json({
        success: false,
...
router.post(
  ...
  async (req: AuthenticatedAdminRequest, res: Response) => {
    try {
      const { storeId } = req.params;
      const body = getValidated<CreateProductInput>(req);
      ...
      const images: ProductImage[] | undefined = body.images?.map((img, idx) => ({
        id: `img-${Date.now()}-${idx}`,
        url: img.url,
        uploadedAt: new Date(),
      }));
      ...
      adapters.logger.error('Failed to create product', error as Error);
...
router.patch(
  ...
  async (req: AuthenticatedAdminRequest, res: Response) => {
    try {
      ...
      const body = getValidated<UpdateProductInput>(req);
      ...
      adapters.logger.error('Failed to update product', error as Error);
 ...
 * Soft-delete produk.
 */
router.delete('/products/:productId', async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    const { productId } = req.params;
    const product = await productService.getProductById(productId);
    ...
    await logAction({
      ...
      entityId: productId,
      adminId: req.admin!.adminId,
      changes: { reason: (req.body?.reason as string) || null, name: product.name },
      ipAddress: req.ip,
    });
    ...
    adapters.logger.error('Failed to delete product', error as Error);
    res.status(500).json({ error: error?.message || 'Failed to delete product' });
  }
});

// ─── GET /api/admin/stores/:storeId/categories — Active categories for store (form dropdown) ───
router.get('/stores/:storeId/categories', async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    const { storeId } = req.params;
    ...
    adapters.logger.error('Failed to fetch categories', error as Error);
