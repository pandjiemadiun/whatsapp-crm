...
router.use((req: AuthenticatedAdminRequest, _res: Response, next: Function) => {
...
router.get('/', async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    ...
    adapters.logger.error('Failed to load magic paste config', error as Error);
 ...
 * Buat pattern baru.
 */
router.post('/patterns', async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
...
changes: newPattern as any,
      ipAddress: req.ip,
    });

    res.status(201).json({ success: true, data: newPattern });
  } catch (error: any) {
    adapters.logger.error('Failed to create pattern', error as Error);
 ...
 */
router.put('/patterns/:id', async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { name, description, regex, fieldMappings, confidence, isActive, sortOrder } = req.body;
    ...
    adapters.logger.error('Failed to update pattern', error as Error);
 ...
 * Hapus pattern.
 */
router.delete('/patterns/:id', async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    const { id } = req.params;
    ...
    await logAction({
      ...
      entityId: id,
      adminId: req.admin!.adminId,
      changes: removed as any,
      ipAddress: req.ip,
    });

    res.json({ success: true, message: 'Pattern berhasil dihapus' });
  } catch (error: any) {
    adapters.logger.error('Failed to delete pattern', error as Error);
 ...
 */
router.put('/settings', async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    ...
    adapters.logger.error('Failed to update settings', error as Error);
