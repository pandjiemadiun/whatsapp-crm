...
router.post('/register', validateRequest(registerAdminSchema, 'body'), adminAuthLimiter, bootstrapRegistrationGate, async (req: Request, res: Response) => {
  try {
    ...
    adapters.logger.error('Admin registration failed', error as Error);
...
router.post('/login', validateRequest(loginSchema, 'body'), adminAuthLimiter, async (req: Request, res: Response) => {
  try {
    ...
    adapters.logger.error('Admin login failed', error as Error);
    res.status(500).json({ error: error?.message || 'Login failed' });
  }
});

// ─── POST /api/admin/auth/logout ───
router.post('/logout', adminAuthMiddleware, async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    ...
    adapters.logger.error('Admin logout failed', error as Error);
    res.status(500).json({ error: error?.message || 'Logout failed' });
  }
});

// ─── GET /api/admin/auth/me ───
router.get('/me', adminAuthMiddleware, async (req: AuthenticatedAdminRequest, res: Response) => {
  try {
    ...
    adapters.logger.error('Admin fetch profile failed', error as Error);
...
router.post(
  ...
  async (req: AuthenticatedAdminRequest, res: Response) => {
    try {
      const { adminEmail, newPassword } = getValidated<{ adminEmail: string; newPassword: string }>(req);
      ...
      adapters.logger.error('Admin password reset failed', error as Error);
