...
router.put('/', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (address !== undefined) {
      if (!address || !address.trim()) return res.status(400).json({ error: 'Alamat tidak boleh dikosongkan' });
    ...
    if (originProvinceId !== undefined) {
      if (!originProvinceId || !originProvinceId.trim()) return res.status(400).json({ error: 'Provinsi tidak boleh dikosongkan' });
      updateData.originProvinceId = originProvinceId.trim();
    }
    if (originProvinceName !== undefined) {
      if (!originProvinceName || !originProvinceName.trim()) return res.status(400).json({ error: 'Nama provinsi tidak boleh dikosongkan' });
      updateData.originProvinceName = originProvinceName.trim();
    }
    if (originCityId !== undefined) {
      if (!originCityId || !originCityId.trim()) return res.status(400).json({ error: 'Kota tidak boleh dikosongkan' });
      updateData.originCityId = originCityId.trim();
    }
    if (originCityName !== undefined) {
      if (!originCityName || !originCityName.trim()) return res.status(400).json({ error: 'Nama kota tidak boleh dikosongkan' });
      updateData.originCityName = originCityName.trim();
    }
    if (originSubdistrictId !== undefined) {
      if (!originSubdistrictId || !originSubdistrictId.trim()) return res.status(400).json({ error: 'Kecamatan tidak boleh dikosongkan' });
      updateData.originSubdistrictId = originSubdistrictId.trim();
    }
    if (originSubdistrictName !== undefined) {
      if (!originSubdistrictName || !originSubdistrictName.trim()) return res.status(400).json({ error: 'Nama kecamatan tidak boleh dikosongkan' });
