...
router.post('/gowa', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (event !== 'message' || payload.is_from_me) return;
    ...
    try {
      ...
      adapters.logger.warn('Customer upsert failed (GOWA)', { conversationId, error: (err as Error).message });
    }
    ...
    adapters.logger.error('GOWA webhook processing error', error as Error);
  }
});

// POST /api/webhooks/fonnte — Receive incoming messages from Fonnte
router.post('/fonnte', async (req: Request, res: Response) => {
  ...
  (async () => {
    try {
      ...
      try {
        ...
        adapters.logger.warn('Customer upsert failed (Fonnte)', { conversationId, error: (err as Error).message });
      }
      ...
      if (customerName) {
        try {
          ...
          adapters.logger.warn('Failed to update conversation customerName', { conversationId, storeId: store.id, error: (err as Error).message });
        }
      }
      ...
      adapters.logger.error('Fonnte webhook processing error', error as Error);
    }
  })();
});
