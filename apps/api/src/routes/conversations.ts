    ...
    if (conversations.length) {
      ...
      for (const c of conversations) {
        ...
        const readAt = m.adminLastReadAt
          ? new Date(m.adminLastReadAt as string)
          : new Date(0);
        (c as any).unreadCount = userMsgs.filter(
          (x) => x.conversationId === c.id && x.createdAt > readAt,
        ).length;
      }
    }
...
router.post('/:id/read', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    adapters.logger.error('Failed to mark conversation read', error as Error);
