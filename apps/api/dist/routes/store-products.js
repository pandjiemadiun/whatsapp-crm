...
import { variantOverrideSchema, createVariantSchema, updateVariantSchema } from '../schemas/index.js';
const router = Router();
...
async function getOwnedProduct(productId, storeId) {
...
router.get('/my', async (req, res) => {
    try {
        ...
        adapters.logger.error('List store products failed', error);
...
router.get('/my/:productId', async (req, res) => {
    try {
        ...
        adapters.logger.error('Get store product detail failed', error);
...
router.get('/my/:productId/variants', async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { productId } = req.params;
        const product = await prisma.product.findFirst({
        ...
        adapters.logger.error('List store product variants failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.post('/my/:productId/variants', validateRequest(createVariantSchema, 'body'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { productId } = req.params;
        ...
        adapters.logger.error('Create store variant failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.patch('/my/variants/:variantId', validateRequest(updateVariantSchema, 'body'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        adapters.logger.error('Update store variant failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.delete('/my/variants/:variantId', async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { variantId } = req.params;
        const variant = await prisma.productVariant.findUnique({
        ...
        adapters.logger.error('Delete store variant failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.post('/my', validateRequest(createStoreProductSchema, 'body'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const input = getValidated(req);
        ...
        adapters.logger.error('Create store product failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.put('/my/:productId', validateRequest(updateStoreProductSchema, 'body'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { productId } = req.params;
        const input = getValidated(req);
        ...
        adapters.logger.error('Update store product failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.delete('/my/:productId', async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { productId } = req.params;
        // Ownership check
        ...
        adapters.logger.error('Delete store product failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.post('/my/magic-paste', validateRequest(magicPasteOwnerSchema, 'body'), async (req, res) => {
    try {
        ...
        adapters.logger.error('Store magic paste failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.post('/my/magic-paste/batch', validateRequest(magicPasteOwnerSchema, 'body'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { text } = getValidated(req);
        const preview = req.query.preview === 'true';
        ...
        adapters.logger.error('Store magic paste batch failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.get('/my/categories', async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        adapters.logger.error('List store categories failed', error);
...
router.post('/my/:productId/image', upload.single('image'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { productId } = req.params;
        ...
        adapters.logger.error('Product image upload failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
router.delete('/my/:productId/image/:index', async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        if (adapters.catalogStorage.refreshImageUrl) {
            refreshedUrls = await Promise.all(images.map(async (img) => ({ ...img, url: await adapters.catalogStorage.refreshImageUrl(img.url) })));
        }
        res.json({ success: true, data: { images: refreshedUrls, primaryImageUrl: images[0]?.url ?? null } });
    }
    catch (error) {
        adapters.logger.error('Product image delete failed', error);
        if (error instanceof ApiError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
...
//# sourceMappingURL=store-products.js.map