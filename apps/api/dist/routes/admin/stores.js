...
router.get('/', validateRequest(queryStoresSchema, 'query'), async (req, res) => {
    try {
        const { page, search, status } = getValidated(req);
        const pageSize = 20;
...
router.put('/:storeId/suspend', async (req, res) => {
    try {
        ...
        adapters.logger.error('Failed to suspend store', error);
...
router.put('/:storeId/activate', async (req, res) => {
    try {
        const { storeId } = req.params;
        const store = await prisma.store.findFirst({
            where: { id: storeId, deletedAt: null },
        ...
        adapters.logger.error('Failed to activate store', error);
...
router.post('/:storeId/reset-password', validateRequest(resetPasswordSchema, 'body'), async (req, res) => {
    try {
        const { storeId } = req.params;
        const { tempPassword: inputPassword } = getValidated(req);
        const store = await prisma.store.findFirst({
            where: { id: storeId, deletedAt: null },
        });
        if (!store) {
        ...
        adapters.logger.error('Failed to reset password', error);
        res.status(500).json({ error: error?.message || 'Failed to reset password' });
    }
});
// ─── POST /api/admin/stores/:storeId/verify-email — Mark store email as verified ───
router.post('/:storeId/verify-email', async (req, res) => {
    try {
        const { storeId } = req.params;
        const store = await prisma.store.findFirst({
            where: { id: storeId, deletedAt: null },
        ...
        adapters.logger.error('Failed to verify email', error);
...
router.post('/:storeId/disconnect-fonnte', async (req, res) => {
    try {
        const { storeId } = req.params;
        const store = await prisma.store.findFirst({
            where: { id: storeId, deletedAt: null },
        ...
        adapters.logger.error('Failed to disconnect Fonnte', error);
...
router.get('/:storeId/gowa-status', async (req, res) => {
    try {
        const { storeId } = req.params;
        ...
        adapters.logger.error('GOWA status check failed', error);
...
router.post('/:storeId/gowa-connect', async (req, res) => {
    try {
        const { storeId } = req.params;
        const cfg = await getGowaConfig();
        const did = deviceId(storeId);
...
router.post('/:storeId/gowa-reset', async (req, res) => {
    try {
        const { storeId } = req.params;
        const cfg = await getGowaConfig();
        const did = deviceId(storeId);
...
router.get('/:storeId', async (req, res) => {
    try {
        ...
        const [totalConversations, totalMessages, totalOrders, aiResponseCount, humanTakeoverCount, costAgg] = await Promise.all([
        ...
        adapters.logger.error('Failed to fetch store detail', error);
