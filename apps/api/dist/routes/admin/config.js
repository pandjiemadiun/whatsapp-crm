...
router.get('/token-usage/last-hour', asyncHandler(async (req, res) => {
...
// Must be before /:key wildcard route.
router.get('/token-usage/query', asyncHandler(async (req, res) => {
...
router.put('/:key', requireAdminRole(['super_admin']), validateRequest(updateConfigSchema, 'body'), asyncHandler(async (req, res) => {
    const { key } = req.params;
    const { value, category, isSecret, description } = getValidated(req);
...
router.delete('/:key', requireAdminRole(['super_admin']), asyncHandler(async (req, res) => {
...
router.post('/reload-cache', requireAdminRole(['super_admin']), asyncHandler(async (req, res) => {
    ...
    switch (service) {
        case 'groq': {
            ...
            if (groqRes.ok) {
                ...
                status = 'unavailable';
                message = `Groq returned HTTP ${groqRes.status}`;
            }
            break;
        }
        case 'gemini': {
