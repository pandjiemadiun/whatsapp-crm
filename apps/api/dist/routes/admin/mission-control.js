...
import { aiOpsQuerySchema, heatmapQuerySchema, leaderboardQuerySchema, } from '../../schemas/missionControl.js';
const router = Router();
/**
 ...
 * Cache: Redis 60s TTL.
 */
router.get('/pulse', asyncHandler(async (_req, res) => {
 ...
 * Cache: Redis 60s TTL.
 */
router.get('/ai-ops', validateRequest(aiOpsQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const { range } = getValidated(req);
 ...
 */
router.get('/heatmap', validateRequest(heatmapQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const { days } = getValidated(req);
 ...
 * Cache: Redis 300s TTL.
 */
router.get('/leaderboard', validateRequest(leaderboardQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const { range } = getValidated(req);
 ...
 * Cache: Redis 60s TTL.
 */
router.get('/wa-status', asyncHandler(async (_req, res) => {
