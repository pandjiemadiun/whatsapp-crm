...
router.post('/dry-run', requireAdminRole(['super_admin']), async (_req, res) => {
    try {
        ...
        adapters.logger.error('Key rotation dry-run failed', error);
...
router.post('/execute', requireAdminRole(['super_admin']), async (req, res) => {
    try {
        ...
        adapters.logger.error('Key rotation failed', error);
        res.status(500).json({
            success: false,
