...
export function buildAdapter(config) {
    ...
    throw new Error(`Unknown AIProviderConfig format: '${config.format}' ` +
...
async function probeProvider(config) {
    ...
    try {
        ...
        return { success: false, latencyMs: 0, errorCategory: ErrorCategory.VALIDATION_ERROR, errorMessage: e.message };
    }
    try {
        const sample = await adapter.generate(TEST_PROMPT);
    ...
    if (existing.isActive) {
        ...
        if (othersActive === 0) {
            return res.status(409).json({
                success: false,
                error: `Cannot delete the only active provider for role '${existing.role}' — ` +
...
export const testConnectionById = async (req, res) => {
    const { id } = req.params;
