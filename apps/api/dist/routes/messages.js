...
function getPublicWebhookBaseUrl(req) {
...
router.post('/handle', async (req, res) => {
    try {
        ...
        const result = await conversationService.processCustomerMessage(storeId, customerId, conversationId, message);
        if (!result) {
            return res.json({
                success: true,
...
router.get('/stats/:conversationId', async (req, res) => {
    try {
        ...
        adapters.logger.error('Stats handler error', error);
...
router.post('/rotate-webhook-secret', async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        adapters.logger.error('Rotate webhook secret error', error);
...
router.get('/webhook-url', async (req, res) => {
    try {
        ...
        adapters.logger.error('Get webhook url error', error);
