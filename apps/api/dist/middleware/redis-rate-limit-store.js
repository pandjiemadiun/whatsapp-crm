...
export class RedisRateLimitStore {
    ...
    async increment(key) {
        try {
            ...
            const totalHits = results ? (results[0][1] || 0) : 0;
            ...
            logger.error('Redis rate limit increment failed', err);
    ...
    async decrement(key) {
        try {
            ...
            logger.error('Redis rate limit decrement failed', err);
        }
    }
    ...
    async resetKey(key) {
        try {
            ...
            logger.error('Redis rate limit resetKey failed', err);
        }
    }
    ...
    async resetAll() {
        try {
            ...
            logger.error('Redis rate limit resetAll failed', err);
        }
    }
    /** Return the current hit count for a key (for monitoring) */
    async getHitCount(key) {
        try {
