...
function customerConvRoom(storeId, conversationId) {
    return `store:${storeId}:conv:${conversationId}`;
}
function adminRoom(storeId) {
    return `store:${storeId}:admin`;
}
async function verifyAdminViaStoreSetting(token) {
...
export class RealtimeService {
    ...
    presenceKey(storeId, conversationId) {
     ...
     */
    isCustomerConversationOnline(storeId, conversationId) {
        const s = this.customerPresence.get(this.presenceKey(storeId, conversationId));
        return !!s && s.size > 0;
    }
    // ---------- Socket.IO middleware ----------
    async authGuard(socket, next) {
        ...
        try {
            ...
            if (token) {
                ...
                let conversationId = q.conversationId ?? null;
                if (conversationId) {
            ...
            adapters.logger.error('WS auth guard error', e);
    ...
    onConnection(socket) {
        ...
        socket.on('admin_typing', (payload) => {
            if (ctx.party !== 'human_agent' || !payload.conversationId)
                return;
        ...
        socket.on('disconnect', () => {
            ...
            if (n <= 0)
                this.onlineByStore.delete(ctx.storeId);
            else
                this.onlineByStore.set(ctx.storeId, n);
            ...
            if (ctx.party === 'customer' && ctx.conversationId) {
                ...
                if (set) {
                    ...
                    if (set.size === 0)
                        this.customerPresence.delete(key);
                }
            }
        });
    }
