 ...
 */
export function classifyStructured(result) {
    if (!result || !result.message?.content) {
        return { messageType: 'text', basePayload: null };
    }
    const meta = result.metadata || {};
    const reason = isNonEmptyString(meta.reason) ? meta.reason : undefined;
    ...
    if (result.source === ResponseSource.PRODUCT && meta.matchedNames) {
        const names = toArray(meta.matchedNames);
        const ids = toArray(meta.productIds);
        const prices = toArray(meta.matchedPrices);
        if (names.length >= 2) {
            return {
    ...
    if (result.source === ResponseSource.CATALOG && Array.isArray(meta.items)) {
        const items = toArray(meta.items);
        if (items.length >= 2) {
            return {
                messageType: 'product_list',
                basePayload: {
 ...
 */
export async function mapStructured(result, conversationId) {
    if (!result || !result.message?.content) {
...
async function fetchClarificationOptions(conversationId) {
...
async function fetchCart(conversationId) {
...
async function enrichProduct(base) {
    ...
    if (prod?.hasVariants) {
        const variants = await productService.getMappedVariants(id, prod.storeId);
