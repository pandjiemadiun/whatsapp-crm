...
export function validate(result, ctx) {
    ...
    for (const a of acts) {
        for (const e of a.entities) {
            if (e.type !== 'product')
                continue; // hanya mention produk dicek katalog
            const v = e.value.toLowerCase();
            if (!catalogNames.has(v) && !unmatchedSet.has(v)) {
                reasons.push(`I-V2-1: mention "${e.value}" (act ${a.act_id}) tidak di catalog dan tidak di unmatched_mentions`);
                retryable = true;
            }
        }
    }
    ...
    if (unmatched.length > 0) {
        ...
        const mentionedInReply = !!result.reply_draft &&
            unmatched.some((u) => result.reply_draft.toLowerCase().includes(u.toLowerCase()));
