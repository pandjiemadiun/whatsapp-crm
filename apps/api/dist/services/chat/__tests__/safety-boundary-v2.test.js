...
const SERVICE_PATH = fileURLToPath(new URL('../../../business/conversation.service.ts', import.meta.url));
/**
    ...
    try {
        ...
        try {
            ...
            if (v2MutationExecuted) {
                ...
                return {
                    safeReplied: true,
                    replyText: 'Baik kak, pesanan Kakak sudah kami catat. Silakan ketik *total* atau *cek pesanan* untuk melihat ringkasannya ya. 🙏',
                    v1FallbackTriggered: false,
                    mutationCount,
                };
            }
...
describe('P0 Safety Boundary — v2 mutates, never falls back to v1', () => {
    ...
    it('REAL SOURCE: safety-boundary guard terpasang di conversation.service.ts', () => {
        ...
        assert.ok(setCount >= 2, `flag harus di-set setelah executeCartOps (resolved + reasoned/cartActs); ditemukan ${setCount}`);
        assert.match(src, /buildSafeReply\(/, 'safe-reply builder harus dipanggil di guard');
    });
});
//# sourceMappingURL=safety-boundary-v2.test.js.map