...
import { logShadowEntry, computeMismatch, buildShadowEntry, } from '../shadow-logger.js';
// ─────────────────────────────────────────────────────────────────────────────
// Factory helpers
// ─────────────────────────────────────────────────────────────────────────────
...
describe('logShadowEntry', () => {
    it('menghasilkan log info dengan field lengkap + stamp version', () => {
        ...
        assert.equal(meta.conversation_id, 'conv-1');
        assert.equal(meta.message_id, 'msg-1');
        assert.equal(meta.store_id, 'store-1');
        assert.ok(meta.old !== undefined);
        assert.ok(meta.new !== undefined);
        assert.ok(meta.mismatch !== undefined);
    });
});
// ─────────────────────────────────────────────────────────────────────────────
...
describe('computeMismatch — reply', () => {
    it('reply sama (case-insensitive) → replyDiffers=false', () => {
        const result = computeMismatch({ reply: 'Hello World', entities: [] }, { reply_draft: 'hello world', entities: [] });
        assert.equal(result.replyDiffers, false);
    });
    it('reply beda → replyDiffers=true', () => {
        const result = computeMismatch({ reply: 'Hello World', entities: [] }, { reply_draft: 'Goodbye World', entities: [] });
        assert.equal(result.replyDiffers, true);
    });
    it('reply_draft null → diperlakukan sebagai string kosong', () => {
        const result = computeMismatch({ reply: '', entities: [] }, { reply_draft: null, entities: [] });
        assert.equal(result.replyDiffers, false);
    });
});
// ─────────────────────────────────────────────────────────────────────────────
...
describe('computeMismatch — entity set', () => {
    it('entity set sama → entitySetDiffers=false', () => {
        ...
        const result = computeMismatch({ reply: 'same', entities: oldEntities }, { reply_draft: 'same', entities: newEntities });
    ...
    it('entitas baru kosong → entitySetDiffers=true, newEntityCount=0', () => {
        const result = computeMismatch({ reply: 'same', entities: [{ type: 'product', value: 'Beras' }] }, { reply_draft: 'same', entities: [] });
        assert.equal(result.entitySetDiffers, true);
...
describe('buildShadowEntry', () => {
    it('menyusun struktur lengkap', () => {
        ...
        assert.deepEqual(entry.new.intents[0].entities, params.reasoningResult.acts[0].entities);
    ...
    it('handle reply_draft undefined → new.reply_draft = null', () => {
        const entry = buildShadowEntry(makeParams({
            ...
            newOutcome: 'fallback_reasoning_failed',
        }));
...
//# sourceMappingURL=shadow-logger-v2.test.js.map