...
describe('understand — LLM reasoning + validator (FASE B4)', () => {
    ...
    it('Validator reject terminal I-V2-6 (low selection confidence) → clarification_trigger, llmCalls=1, JANGAN retry', async () => {
        ...
        if (result.outcome === 'reasoned') {
            ...
            assert.equal(result.trace.steps.some((s) => s.step === 'fallback'), false);
        }
    });
});
// ─────────────────────────────────────────────────────────────────────────────
...
describe('understand — TASK 8 regression A/B/C (mock LLM)', () => {
    ...
    it('B: "Saya mau pesan, kamu jual apa saja" → fast path tier (katalog), 0 LLM', async () => {
        const ws = makeWorkspace({ pendings: [] });
        mockResponses = [JSON.stringify(makeValidResult())];
        const result = await understand('Saya mau pesan, kamu jual apa saja', ws, CATALOG, [], makeStubFallback(ResponseSource.CATALOG) // stub tier katalog = hit
        );
        assert.equal(llmCalls, 0);
        assert.equal(result.outcome, 'tier');
    });
    it('C: "Eh beras ga jadi" (cancel) → LLM intent cancel, plannedActs berisi cancel act', async () => {
        ...
        if (result.outcome === 'reasoned') {
            ...
            assert.ok(result.plannedActs.some((a) => a.intent.toLowerCase().includes('cancel') ||
                a.intent.toLowerCase().includes('remove')), 'plannedActs harus berisi act pembatalan');
        }
    });
});
