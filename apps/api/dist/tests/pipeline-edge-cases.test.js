...
import { isDeadEnd, isDeadEndWithContext, isUrgent, isOrderFunnelContext, messageQueueService } from '../services/message-queue.service.js';
    ...
    test('Conversation human_takeover status dilanggengkan meski circuit breaker reset', async () => {
        ...
        try {
            await prisma.conversation.upsert({
                ...
                update: { status: 'human_takeover', humanTakeoverAt: new Date() },
                create: {
                    id: TEST_CONV_ID,
                    storeId: ec2StoreId,
...
describe('EDGE CASE 3: Multi-modal Coalescing', () => {
    test('Image + caption text dalam 3s → di-buffer bersama', async () => {
        ...
        assert.ok(stats2.pendingMediaBuffers <= 1, `Media buffers should not exceed 1: got ${stats2.pendingMediaBuffers}`);
        messageQueueService.cleanup();
    });
...
//# sourceMappingURL=pipeline-edge-cases.test.js.map