...
let origUpdate;
const capturedRef = { env: null };
let unsubMessageCreated = null;
...
const testPendingClarification = {
...
function makeResult(opts) {
    ...
    if (opts.cartOpsExecuted !== undefined)
        metadata.cartOpsExecuted = opts.cartOpsExecuted;
    ...
    if (!opts.reason && !opts.matchedNames)
...
async function seedRow(result, existingMeta = null) {
...
function withStub(obj, k, stub, fn) {
    const orig = obj[k];
    obj[k] = stub;
    return fn().finally(() => { obj[k] = orig; });
}
...
test('FASE 2 structured-message', async (t) => {
    ...
    unsubMessageCreated = eventBus.subscribe('message.created', (env) => {
        capturedRef.env = env;
    });
    const stubEngine = (result) => {
        conversationService.processCustomerMessage = (async () => result);
    };
    const restoreEngine = () => {
    ...
    try {
        ...
        await t.test('T3-list classifyStructured PRODUCT + 2 matchedNames → product_list', () => {
            ...
            assert.equal((s.basePayload?.items).length, 2);
        });
        ...
        await t.test('T2-int quick_reply payload.options == authoritative context options', async () => {
            ...
            try {
                await withStub(canonicalConversationStateService, 'getV1PendingClarification', (async () => pendingStub), async () => {
                    ...
                    assert.equal(res.type, 'quick_reply');
                    ...
                    assert.deepEqual(row.metadata?.messagePayload?.options, options);
                    const ws = capturedRef.env?.data;
        ...
        await t.test('T2b-int clarification_asked but NO options → text (empty quick_reply disallowed)', async () => {
            ...
            try {
                ...
                await withStub(canonicalConversationStateService, 'getV1PendingClarification', (async () => null), async () => {
                    const res = await conversationDeliveryService.processWebRequest({
                        storeId: STORE_ID, customerId: CUST_ID, conversationId: CONV_ID, message: 'msg',
                    });
                    assert.equal(res.kind, 'ok');
                    assert.equal(res.type, 'text'); // downgrade: tidak boleh quick_reply kosong
                    assert.equal(res.payload, null);
                });
                // also explicit: pendingClarification ada tapi options kosong
                await withStub(canonicalConversationStateService, 'getV1PendingClarification', (async () => ({ question: 'q', options: [], expected_type: 'choice', retry_count: 0 })), async () => {
                    const res = await conversationDeliveryService.processWebRequest({
                        storeId: STORE_ID, customerId: CUST_ID, conversationId: CONV_ID, message: 'msg',
                    });
                    assert.equal(res.type, 'text'); // [] juga → text
                });
            }
            finally {
                restoreEngine();
        ...
        await t.test('T4-int cart payload.items + total == authoritative draft order', async () => {
            ...
            capturedRef.env = null;
            try {
                await withStub(orderService, 'getOrdersByConversation', (async () => orders), async () => {
                    const res = await conversationDeliveryService.processWebRequest({
                        storeId: STORE_ID, customerId: CUST_ID, conversationId: CONV_ID, message: 'msg',
                    });
                    assert.equal(res.kind, 'ok');
                    assert.equal(res.type, 'cart');
                    ...
                    assert.deepEqual(row.metadata?.messagePayload?.items, orders[0].items);
                });
            }
            finally {
                restoreEngine();
        ...
        await t.test('T4b-int modify_cart but empty cart → downgrade to text', async () => {
            ...
            capturedRef.env = null;
            try {
                await withStub(orderService, 'getOrdersByConversation', (async () => [{ orderStatus: 'draft', totalPrice: 0, items: [] }]), async () => {
                    const res = await conversationDeliveryService.processWebRequest({
                        storeId: STORE_ID, customerId: CUST_ID, conversationId: CONV_ID, message: 'msg',
                    });
                    assert.equal(res.kind, 'ok');
        ...
        await t.test('T3-int product payload == {id,name,price,stock,imageUrl} from authoritative match + DB', async () => {
            ...
            capturedRef.env = null;
            try {
                await withStub(productService, 'getProductById', (async (_id) => prod), async () => {
                    const res = await conversationDeliveryService.processWebRequest({
                        storeId: STORE_ID, customerId: CUST_ID, conversationId: CONV_ID, message: 'msg',
                    });
                    assert.equal(res.kind, 'ok');
        ...
        await t.test('T3-list-int product_list payload.items enriched with stock+imageUrl', async () => {
            ...
            capturedRef.env = null;
            try {
                await withStub(productService, 'getProductById', (async (id) => byId[id]), async () => {
                    const res = await conversationDeliveryService.processWebRequest({
                        storeId: STORE_ID, customerId: CUST_ID, conversationId: CONV_ID, message: 'msg',
                    });
                    assert.equal(res.kind, 'ok');
                    assert.equal(res.type, 'product_list');
        ...
        await t.test('T7 existing metadata preserved + messagePayload attached', async () => {
            ...
            stubEngine(r);
            try {
                await withStub(canonicalConversationStateService, 'getV1PendingClarification', (async () => testPendingClarification), async () => {
                    const res = await conversationDeliveryService.processWebRequest({
        ...
        await t.test('T9 DB id = HTTP messageId = WS data.id (same id)', async () => {
            ...
            try {
                await withStub(canonicalConversationStateService, 'getV1PendingClarification', (async () => testPendingClarification), async () => {
                    ...
                    const wsId = capturedRef.env?.data?.id ?? null;
        ...
        await t.test('T14 structured UPDATE fails → fallback text, no second insert', async () => {
            ...
            prisma.conversationHistory.update = (async () => { throw new Error('PROBE_UPDATE_FAIL'); });
            try {
                const res = await conversationDeliveryService.processWebRequest({
                    storeId: STORE_ID, customerId: CUST_ID, conversationId: CONV_ID, message: 'msg',
                });
                assert.equal(res.kind, 'ok');
                assert.equal(res.type, 'text'); // fallback
                assert.equal(res.payload, null);
                const row = await fetchRow(r.message.id);
...
//# sourceMappingURL=structured-message.test.js.map