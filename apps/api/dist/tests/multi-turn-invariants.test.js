...
import { decideRoute, buildRouteContext } from '../business/route-decider.js';
...
const multiTurnCases = dataset.filter((c) => c.turns).map((c) => ({
    name: c.name,
...
// ── Invariant checks ──
function checkI1(content) {
    if (content.includes('Rp 0'))
        return 'I1: Response contains "Rp 0"';
    return null;
}
function checkI2(cartAfter, cartBefore, msg) {
...
function checkI7(content, source, route) {
    if (source === 'human') {
...
function checkI6(content, storeId) {
...
function checkI3(route, source, msg) {
...
function checkI4(content, cartAfter) {
...
function checkI5(route, msg, hasPreviousMutation) {
...
async function seedConversation(testCase) {
...
async function setCart(convId, items) {
...
async function setActiveOrder(convId, storeId, order) {
    await prisma.order.upsert({
        ...
        create: {
            ...
            customerId: 'multiturn-tester',
            items: order.items,
            totalPrice: order.items.reduce((s, i) => s + (i.price || 0) * (i.quantity || 1), 0),
...
describe('Multi-Turn Invariant Tests — ' + TOTAL_CASES + ' transcripts', () => {
    before(async () => {
        try {
            ...
            adapters.logger.warn('Redis flush failed', { error: e.message });
        }
    });
    after(async () => {
        ...
        test(testCase.name + ' — ' + testCase.description, async (t) => {
            ...
            for (let i = 0; i < testCase.turns.length; i++) {
                ...
                if (route.kind === 'cart_modify' && route.remove) {
                    ...
                    assert.ok(cartAfterDb.length < cartBefore.length || cartAfterDb.length === 0, `Turn ${i + 1}: cart should be modified`);
                }
                ...
                if (!route.kind.startsWith('cart') || (route.kind === 'cart_modify' && route.remove)) {
                    ...
                    if (responseContent) {
                        ...
                        if (i1Error)
                            assert.fail(`Turn ${i + 1} "${msg}": ${i1Error}`);
                    }
                    ...
                    if (turn.expect.source && responseSource) {
                        ...
                        assert.ok(sourceOk, `Turn ${i + 1} "${msg}": expected source "${turn.expect.source}" but got "${responseSource}"`);
                    }
                    // mustContain
                    if (turn.expect.mustContain && responseContent) {
                        for (const expected of turn.expect.mustContain) {
                            assert.ok(responseContent.toLowerCase().includes(expected.toLowerCase()), `Turn ${i + 1} "${msg}": response must contain "${expected}"`);
                        }
                    }
                    // mustNotContain
                    if (turn.expect.mustNotContain && responseContent) {
                        for (const notExpected of turn.expect.mustNotContain) {
                            assert.ok(!responseContent.toLowerCase().includes(notExpected.toLowerCase()), `Turn ${i + 1}: response must NOT contain "${notExpected}"`);
                        }
                    }
                }
...
describe('BAGIAN 4: Invariants I8-I15 (Chat-Flow Arsitektur Satu Panggilan)', () => {
    ...
    test('I9: klarifikasi tidak muncul 2x berurutan — retry_count maks 1', async () => {
        ...
        const result1 = resolvePendingClarification('yang murah', pending);
        ...
        const result2 = resolvePendingClarification('yang enak', pending2);
    ...
    test('I10: afirmatif menutup klarifikasi dengan 0 LLM', async () => {
        ...
        const pending = {
            ...
            options: [{ id: '1', label: 'Checkout', cartOps: [{ type: 'add', product: 'Beras', qty: 1 }] },
                { id: '2', label: 'Tambah', cartOps: [{ type: 'add', product: 'Minyak', qty: 1 }] }],
        ...
        const result = resolvePendingClarification('dua dua', pending);
    ...
    test('I14: pending clarification resolved tidak memicu LLM', async () => {
        ...
        const result = resolvePendingClarification('ya', pending);
        ...
        const result2 = resolvePendingClarification('gak jadi', pending2);
    ...
    test('I15: cart_ops dari LLM wajib divalidasi terhadap DB', async () => {
        ...
        const llmCartOps = [{ type: 'add', product: 'ProdukHantu', qty: 1, price: 999999 }];
        const dbProducts = ['Beras', 'Wortel', 'Kentang'];
        // Validation: hanya terima produk yang ada di DB
        const validOps = llmCartOps.filter((op) => dbProducts.some((p) => p.toLowerCase() === op.product.toLowerCase()));
        assert.equal(validOps.length, 0, 'Non-DB product must be rejected');
        // Produk valid
        const validLlmOps = [{ type: 'add', product: 'Beras', qty: 1 }];
        const validOps2 = validLlmOps.filter((op) => dbProducts.some((p) => p.toLowerCase() === op.product.toLowerCase()));
...
describe('BAGIAN 4: Invariant I16 (Multi-key Groq fallback)', () => {
    ...
    test('I16c: parseKeys validates minimal 1 key', async () => {
        ...
        assert.deepEqual(router.parseKeys('key1, key2 , key3'), ['key1', 'key2', 'key3']);
    });
...
//# sourceMappingURL=multi-turn-invariants.test.js.map