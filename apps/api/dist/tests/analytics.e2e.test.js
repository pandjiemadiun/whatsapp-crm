...
before(async () => {
    ...
    const baseEntities = (name, confidence) => ({ name, price: 10000, confidence });
...
test('5. Trend: record hari ini masuk bucket hari ini (bukan pindah hari)', async () => {
    ...
    const todayBucket = body.data.trend.find((t) => t.date === todayKey);
...
test('6. Filter status=success → hanya 2 record', async () => {
    ...
    assert.ok(body.data.history.every((h) => h.status === 'success'));
});
...
test('9. History: kolom extractedName & errorMessage konsisten', async () => {
    ...
    const body = await parseJson(res);
    const names = body.data.history.map((h) => h.extractedName).sort();
    assert.deepEqual(names, ['Malam Product', 'Pagi Product', 'Siang Product']);
    const failed = body.data.history.find((h) => h.status === 'failed');
    assert.ok(failed);
