...
function makeMockProvider(name, behavior) {
    const provider = {
        ...
        generate: async (_prompt, _options) => {
            provider.callCount++;
            if (behavior === 'timeout') {
                throw new AIProviderError(`${name} request timeout`, ErrorCategory.NETWORK_TIMEOUT, name, undefined, true);
            }
            if (behavior === 'fail') {
                throw new AIProviderError(`${name} server error`, ErrorCategory.SERVER_ERROR, name, 500, true);
            }
            if (behavior === 'slow') {
                return new Promise(() => { });
            }
...
describe('LLMGateway', () => {
    ...
    test('provider failure — both fail throws AIProviderError', async () => {
        ...
        const gatekeeper = mockGatekeeper;
        const gateway = new LLMGateway(primary, fallback, gatekeeper, 5000, 1);
        await assert.rejects(() => gateway.generate('hello'), (err) => err instanceof AIProviderError);
        const stats = gateway.getStats();
    ...
    test('gateway mockability — generate is replaceable', async () => {
        ...
        gateway.generate = async () => mockResponse;
        ...
        // Restore
        gateway.generate = originalGenerate;
    ...
    test('circuit breaker opens after threshold failures', async () => {
        ...
        await assert.rejects(() => gateway.generate('should-not-attempt'), (err) => err instanceof CircuitOpenError);
    });
