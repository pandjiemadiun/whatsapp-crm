...
test('21. Stock: unit-based "per ikat, stok 50" → null + warning', async () => {
    ...
    assert.ok(body.data.warning.some((w) => /Stock ambiguous/i.test(w)));
});
...
test('23. Category: not found → null + warning', async () => {
    ...
    assert.ok(body.data.warning.some((w) => /Category.*not found/i.test(w)));
});
...
test('25. Confidence: low (< 0.8) → warning', async () => {
    ...
    assert.equal(res.status, 201);
    const body = await parseJson(res);
    assert.ok(body.data.warning.some((w) => /confidence low/i.test(w)));
});
...
//# sourceMappingURL=products-magic-paste.e2e.test.js.map