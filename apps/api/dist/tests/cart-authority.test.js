...
before(async () => {
    ...
    storeProducts = products.map((p) => ({ id: p.id, name: p.name, price: p.price }));
});
after(async () => {
    await cleanup();
...
test('remove: line item not in cart throws CartInvariantError', async () => {
    const convId = await createConversation();
    await cartAuthority.addLine(convId, storeId, customerId, storeProducts[0].id, 1);
    await assert.rejects(() => cartAuthority.removeLine(convId, 'nonexistent-item-id'), (err) => err instanceof CartInvariantError && err.code === 'ITEM_NOT_FOUND');
});
...
test('updateQuantity: negative qty throws', async () => {
    const convId = await createConversation();
    const lines = await cartAuthority.addLine(convId, storeId, customerId, storeProducts[0].id, 1);
    await assert.rejects(() => cartAuthority.updateQuantity(convId, lines[0].id, -1), (err) => err instanceof CartInvariantError && err.code === 'INVALID_QUANTITY');
});
// ── READ / WRITE: Clear ─────────────────────────────────────────────────────
...
test('addLine: product from different store throws cross-tenant error', async () => {
    const convId = await createConversation();
    await assert.rejects(() => cartAuthority.addLine(convId, storeId, customerId, `${TEST_PREFIX}-prod-otherstore`, 1), (err) => err instanceof CartInvariantError && err.code === 'CROSS_TENANT');
});
test('addLine: deleted product throws product not available', async () => {
    const convId = await createConversation();
    await assert.rejects(() => cartAuthority.addLine(convId, storeId, customerId, `${TEST_PREFIX}-prod-deleted`, 1), (err) => err instanceof Error);
});
test('addLine: inactive product throws product not available', async () => {
    const convId = await createConversation();
    await assert.rejects(() => cartAuthority.addLine(convId, storeId, customerId, `${TEST_PREFIX}-prod-inactive`, 1), (err) => err instanceof Error);
});
test('addLine: insufficient stock throws', async () => {
    const convId = await createConversation();
    // beras has stock=5, try to add 10
    await assert.rejects(() => cartAuthority.addLine(convId, storeId, customerId, `${TEST_PREFIX}-prod-beras`, 10), (err) => err instanceof CartInvariantError && err.code === 'INSUFFICIENT_STOCK');
});
test('addLine: qty < 1 throws invalid quantity', async () => {
    const convId = await createConversation();
    await assert.rejects(() => cartAuthority.addLine(convId, storeId, customerId, storeProducts[0].id, 0), (err) => err instanceof CartInvariantError && err.code === 'INVALID_QUANTITY');
});
...
test('checkout: no draft order throws', async () => {
    const convId = await createConversation();
    await assert.rejects(() => cartAuthority.checkout(convId, storeId), (err) => err instanceof CartInvariantError && err.code === 'CART_NOT_FOUND');
});
test('checkout: cross-store checkout throws', async () => {
    const convId = await createConversation();
    await cartAuthority.addLine(convId, storeId, customerId, storeProducts[0].id, 1);
    await assert.rejects(() => cartAuthority.checkout(convId, 'wrong-store-id'), (err) => err instanceof CartInvariantError && err.code === 'CART_NOT_FOUND');
});
...
test('executeOps: add + remove in same batch', async () => {
    ...
    const result = await cartAuthority.executeOps([
        { type: 'add', product: 'Wortel', qty: 2 },
        { type: 'remove', product: 'Ayam Goreng' },
...
test('executeOps: product not found is skipped (not fatal)', async () => {
    const convId = await createConversation();
    const result = await cartAuthority.executeOps([{ type: 'add', product: 'Produk Tidak Ada', qty: 1 }], storeId, customerId, convId);
    assert.equal(result.length, 0);
});
test('executeOps: repeated add increments qty', async () => {
    const convId = await createConversation();
    await cartAuthority.executeOps([{ type: 'add', product: 'Ayam Goreng', qty: 1 }], storeId, customerId, convId);
    const result = await cartAuthority.executeOps([{ type: 'add', product: 'Ayam Goreng', qty: 2 }], storeId, customerId, convId);
    assert.equal(result.length, 1);
...
describe('G2-C Cleanup: Representation Consistency', () => {
    ...
    test('ADD (executeOps): all 3 representations consistent', async () => {
        const convId = await createConversation();
        await cartAuthority.executeOps([{ type: 'add', product: 'Ayam Goreng', qty: 3 }], storeId, customerId, convId);
        const orderItems = await prisma.orderItem.findMany({
            where: { order: { conversationId: convId, orderStatus: 'draft' } },
        });
        const order = await prisma.order.findFirst({
        ...
        assert.equal((ctxRow?.extractedEntities?.confirmedItems || []).length, 1);
    ...
    test('UPDATE: all 3 representations consistent', async () => {
        ...
        assert.equal((order?.items)[0].qty, 5);
        assert.equal((ctxRow?.extractedEntities?.confirmedItems || [])[0].qty, 5);
    ...
    test('CLEAR: all 3 representations empty after clearCart', async () => {
        ...
        assert.equal((ctxRow?.extractedEntities?.confirmedItems || []).length, 0);
...
describe('G2-C Cleanup: Product Name Resolution', () => {
    test('exact match: "Minyak Goreng" resolves uniquely', async () => {
        const convId = await createConversation();
        const result = await cartAuthority.executeOps([{ type: 'add', product: 'Minyak Goreng', qty: 1 }], storeId, customerId, convId);
        assert.equal(result.length, 1);
    ...
    test('substring: "minyak" matches 3 products → ambiguous (no mutation)', async () => {
        const convId = await createConversation();
        const result = await cartAuthority.executeOps([{ type: 'add', product: 'minyak', qty: 1 }], storeId, customerId, convId);
    ...
    test('substring: "minyak 1" matches 1 product → resolves', async () => {
        const convId = await createConversation();
        const result = await cartAuthority.executeOps([{ type: 'add', product: 'minyak 1', qty: 1 }], storeId, customerId, convId);
        assert.equal(result.length, 1);
    ...
    test('substring: "minyak 1 liter" matches 1 product → resolves', async () => {
        const convId = await createConversation();
        const result = await cartAuthority.executeOps([{ type: 'add', product: 'minyak 1 liter', qty: 1 }], storeId, customerId, convId);
        assert.equal(result.length, 1);
        assert.equal(result[0].product, 'Minyak 1 Liter');
    });
    test('substring: "minyak goreng 2" matches 0 products → not found (skipped)', async () => {
        const convId = await createConversation();
        const result = await cartAuthority.executeOps([{ type: 'add', product: 'minyak goreng 2', qty: 1 }], storeId, customerId, convId);
        assert.equal(result.length, 0);
    });
    test('cross-store: "minyak" from other store → different results (tenant-scoped)', async () => {
        ...
        const result = await cartAuthority.executeOps([{ type: 'add', product: 'Produk Toko Lain', qty: 1 }], storeId, customerId, convId);
...
describe('G2-C Cleanup: Stock Concurrency', () => {
    test('addLine: stock=1, add 1 succeeds, add 1 again fails (insufficient)', async () => {
        ...
        // 5+1=6 > stock=5
        await assert.rejects(() => cartAuthority.addLine(convId, storeId, customerId, `${TEST_PREFIX}-prod-beras`, 1), (err) => err instanceof CartInvariantError && err.code === 'INSUFFICIENT_STOCK');
    });
    test('executeOps: stock=5, add 5 in first batch, add 1 in second batch fails', async () => {
        const convId = await createConversation();
        await cartAuthority.executeOps([{ type: 'add', product: 'Beras', qty: 5 }], storeId, customerId, convId);
        // All 5 in stock — now add 1 more should fail
        await cartAuthority.executeOps([{ type: 'add', product: 'Beras', qty: 1 }], storeId, customerId, convId);
    ...
    test('checkout: enforces final stock invariant at cart→order boundary', async () => {
        ...
        await assert.rejects(() => cartAuthority.checkout(convId, storeId), (err) => err instanceof CartInvariantError && err.code === 'INSUFFICIENT_STOCK');
        // Restore stock
        await prisma.product.update({
            where: { id: `${TEST_PREFIX}-prod-beras` },
            data: { stock: 5 },
...
//# sourceMappingURL=cart-authority.test.js.map