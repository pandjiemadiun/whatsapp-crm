...
function stubLLM() {
    adapters.ai.generate = async (prompt, _options) => {
...
test('3. Preview batch: campuran sukses + gagal (baris tanpa harga)', async () => {
    ...
    const failed = body.data.items.find((it) => it.status === 'failed');
    assert.ok(failed);
...
test('5. Preview batch: baris > 500 char → skipped', async () => {
    ...
    const skipped = body.data.items.find((it) => it.status === 'skipped');
    assert.ok(skipped);
...
//# sourceMappingURL=batch-magic-paste.e2e.test.js.map