...
async function makeOrder(customerId, sId, opts = {}) {
...
test('b. payment-report paymentMethod=cod -> 400, no DB write', async () => {
    const o = await makeOrder(custA, storeId);
    await assert.rejects(() => paymentService.reportPayment(o.id, storeId, custA, 'cod', 'x'), (e) => e.statusCode === 400);
...
test('c. payment-report paymentStatus already paid -> 400', async () => {
    ...
    await assert.rejects(() => paymentService.reportPayment(o.id, storeId, custA, 'transfer', 'x'), (e) => e.statusCode === 400);
    const after = await prisma.order.findUnique({ where: { id: o.id } });
...
test('d. payment-verify paymentMethod=cod -> 400', async () => {
    ...
    await assert.rejects(() => paymentService.verifyPayment(o.id, storeId, 'approve', 'paid', storeId), (e) => e.statusCode === 400);
});
...
test('g. payment-verify approve invalid target -> full rollback', async () => {
    ...
    await assert.rejects(() => paymentService.verifyPayment(o.id, storeId, 'approve', 'completed', storeId), (e) => e.statusCode === 400);
    const after = await prisma.order.findUnique({ where: { id: o.id } });
...
test('h. payment-verify approve without targetOrderStatus -> 400', async () => {
    ...
    await assert.rejects(() => paymentService.verifyPayment(o.id, storeId, 'approve', undefined, storeId), (e) => e.statusCode === 400);
});
...
test('j. tenant isolation — order milik store/customer lain ditolak', async () => {
    ...
    await assert.rejects(() => paymentService.reportPayment(o.id, store2Id, custA, 'transfer', 'x'), (e) => e.code === 'ERR_NOT_FOUND');
    await assert.rejects(() => paymentService.reportPayment(o.id, storeId, 'wrong-cust', 'transfer', 'x'), (e) => e.code === 'ERR_NOT_FOUND');
    await assert.rejects(() => paymentService.verifyPayment(o.id, store2Id, 'approve', 'paid', store2Id), (e) => e.code === 'ERR_NOT_FOUND');
