...
export class HealthService {
    ...
    async checkRedis() {
        ...
        try {
            ...
            if (!ok)
                return { status: 'down', responseTime: Date.now() - start, error: 'PING failed' };
            return { status: 'ok', responseTime: Date.now() - start, lastCheck: new Date().toISOString() };
        }
        catch (error) {
            return { status: 'down', error: error.message };
    ...
    async getMetrics() {
        ...
        const [totalStores, activeStores, totalConversations, totalMessages, aiResponsesLast24h, avgResponseTimeAgg, fonnteStores,] = await Promise.all([
...
export function startHealthCheckInterval() {
    setInterval(async () => {
        try {
            ...
            adapters.logger.warn('Background health check failed', error);
        }
    }, 30000);
}
