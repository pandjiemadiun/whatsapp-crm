...
export class OrderService {
     ...
     */
    async cancelOrder(orderId, storeId, customerId, options) {
        ...
        if (order.storeId !== storeId || order.customerId !== customerId) {
            const err = new Error(`Order ${orderId} does not belong to store ${storeId} / customer ${customerId}`);
 ...
 */
export function shouldRestoreStock(orderStatus) {
    return PRE_SHIPMENT_STATUSES.includes(orderStatus);
}
/**
 ...
 */
export async function restoreStockForOrderItems(items, tx) {
    const client = (tx ?? prisma);
    for (const item of items) {
        if (!item.productId)
            continue; // no product = nothing to restore
        if (item.variantId) {
