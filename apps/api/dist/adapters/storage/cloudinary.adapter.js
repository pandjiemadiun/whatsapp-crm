    ...
    async uploadImage(buffer, folder) {
        ...
        return new Promise((resolve, reject) => {
            const stream = cloudinary.uploader.upload_stream({ folder, resource_type: 'image', transformation: [{ width: 512, height: 512, crop: 'limit', quality: 80 }] }, (error, result) => {
                if (error || !result) {
                    adapters.logger.error('Cloudinary upload failed', error);
...
export let storageAdapter = selectStorageAdapterSync();
function selectStorageAdapterSync() {
...
export async function reconfigureStorage() {
 ...
 */
export const catalogStorage = r2Adapter;
export const profileStorage = cloudinaryAdapter;
