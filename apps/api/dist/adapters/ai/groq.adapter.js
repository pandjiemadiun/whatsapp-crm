...
export class GroqAdapter {
    constructor(apiKey = process.env.GROQ_API_KEY || process.env.GROQ_API_KEYS?.split(',')[0] || '') {
        this._model = 'openai/gpt-oss-120b';
        this.apiKey = apiKey;
        ...
        if (envKeys) {
            ...
            aiKeyRouter.loadKeys(envKeys).catch((e) => {
                console.warn('[GroqAdapter] Failed to load keys from env:', e.message);
            });
        }
    }
    configure(apiKey) {
    ...
    configureKeys(keys) {
        ...
        aiKeyRouter.loadKeys(keys).catch((e) => {
            console.warn('[GroqAdapter] Failed to load keys:', e.message);
        });
    }
    ...
    async generate(prompt, options) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            ...
            if (!currentKey) {
                ...
                throw new AIProviderError('All Groq API keys are in cooldown', ErrorCategory.RATE_LIMIT, 'groq', 429, true);
            }
            try {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 429) {
                        ...
                        lastError = new AIProviderError(providerError
                            ? `Groq 429 on key ${currentKey.slice(0, 8)}...\nprovider_error=${providerError}`
                            : `Groq 429 on key ${currentKey.slice(0, 8)}...`, ErrorCategory.RATE_LIMIT, 'groq', response.status, true, retryAfter);
                    ...
                    throw new AIProviderError(providerError
                        ...
                        : `Groq API returned ${response.status}: ${response.statusText}`, errorCategory, 'groq', response.status, false, retryAfter);
                }
                ...
                if (!data.choices ||
                    ...
                    throw new AIProviderError('Invalid response structure from Groq API', ErrorCategory.UNKNOWN, 'groq', undefined, false);
                }
                ...
                if (error instanceof Error && error.name === 'AbortError') {
                    ...
                    throw new AIProviderError(`Groq request timeout after ${REQUEST_TIMEOUT_MS}ms`, ErrorCategory.NETWORK_TIMEOUT, 'groq', undefined, false // timeout is not retryable within this adapter — manager handles fallback
                    );
                }
                ...
                throw new AIProviderError(`Groq unexpected error: ${error.message}`, ErrorCategory.UNKNOWN, 'groq', undefined, false, undefined);
            }
        }
        // All keys exhausted (loop finished without return)
        if (lastError)
            throw lastError;
        throw new AIProviderError('All Groq API keys exhausted or unavailable', ErrorCategory.RATE_LIMIT, 'groq', 429, false);
    }
...
JSON Response Format:
{
  ...
  "reasoning": "brief explanation"
        ...
        try {
            ...
            console.warn('[Groq] Intent extraction failed, returning default fallback intent:', err.message);
            return {
     ...
     */
    async extractProviderError(response) {
