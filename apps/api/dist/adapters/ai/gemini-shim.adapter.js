...
export class GeminiShimAdapter {
    constructor(config) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new AIProviderError(providerError
                    ...
                    : `Gemini API returned ${response.status}: ${response.statusText}`, category, this.name, response.status, this.isRetryable(category), retryAfter);
            }
            ...
            if (!data.candidates ||
                ...
                throw new AIProviderError('Invalid response structure from Gemini API', ErrorCategory.UNKNOWN, this.name, undefined, false);
            }
            ...
            const inputCost = (promptTokenCount / 1000000) * this.inputPricePer1M;
            const outputCost = (totalOutputTokens / 1000000) * this.outputPricePer1M;
            ...
            if (error instanceof Error && error.name === 'AbortError') {
                throw new AIProviderError(`Gemini request timeout after ${this.timeoutMs}ms`, ErrorCategory.NETWORK_TIMEOUT, this.name, undefined, true);
            }
            if (error instanceof AIProviderError)
                throw error;
            throw new AIProviderError(`Gemini unexpected error: ${error.message}`, ErrorCategory.UNKNOWN, this.name, undefined, false);
        }
        finally {
    ...
    async isHealthy() {
        try {
            ...
            const timeoutId = setTimeout(() => controller.abort(), this.timeoutMs);
            try {
                const response = await fetch(`${this.baseUrl}/models?key=${this.apiKey}`, { signal: AbortSignal.timeout ? AbortSignal.timeout(this.timeoutMs) : controller.signal });
                return response.ok;
            }
            finally {
                clearTimeout(timeoutId);
...
//# sourceMappingURL=gemini-shim.adapter.js.map