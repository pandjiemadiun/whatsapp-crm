...
> **⚠️ PENTING — ini SATU-SATUNYA entri retroaktif yang secara eksplisit mengaku "tidak ada commit".** Dituliskan demi kejujuran: RAILS §10.2 ("unreported work gap") justru memperingatkan bahwa kejadian yang tidak tercatat di git tidak boleh disamarkan seolah tercatat. Setiap baris "Commit:" di bawah ini adalah `eac743d` kecuali dinyatakan lain. **JANGAN** menamakan hash per-event — tidak ada.

**Catatan SHA — baseline / precursors / final (yang real, diverifikasi `git log -g --since 2026-09-08 --until 2026-09-15T12:00Z` + `git show -s` tiap hash):**
- Baseline (sebelum jendela): `315861e` — 2026-09-09T14:35:05Z — `docs: add BUG-AUDIT-REPORT plus UNIT6B-UNIT4 & VERIFY-PROVIDER-MULTIPLICITY reports` (tutupan UNIT6-B).
- Precursors UNIT6-B (Sep 8–9, *konteks sebelum*, bukan bagian retro ini; dicantumkan agar rantai jejak real utuh): `2687554` (PWA idempotency), `613840b`/`dda59e1` (surface unresolved), `5c4220a`·`1e84985` (stable clientMsgId), `ac5dca1` (Unit 2a WaCartMutationResult), `b8195c7` (cleanup hardcode DATABASE_URL), `49eb77e` (thread messageId), `1429817` (mapV2ActionsToCartOps Unit 2), `fdde243` (Unit 3 wire script), `de7a7c5` (scope cooldown per role), `0598046` (WIZARD-P1-SCHEMA), `e06abfe` (pw reset wrapper).
- Precursor circuit-breaker visibility (di luar jendela, terkait event #2): `552d489` — 2026-09-05T16:51:17Z — `fix(llm-gateway): crash when all providers in cooldown (BUGFIX A.1/A.2)`. **Bukan** fix event #2, hanya konteks.
- Final (bundle semua kejadian #1–#13): **`eac743d`** — 2026-09-15T06:41:31Z — `feat(v2-engine): v2-rewrite ACTIVE cutover + anti-hallucination mapper fix + docs`.
- Event #14 = `eac743d` itu sendiri (re-aktivasi FINAL).

Konteks:
Jendela 10–14 Sep 2026 adalah **fase rekonstrusi penuh v2-rewrite** — bukan sekadar "cutover." `git log` (termasuk `git log -g` reflog) **tidak menemukan commit satu pun** di jendela 10–14 Sep: semua pekerjaan dikerjakan langsung di working tree tanpa commit, lalu **dibundel lewat `git add -A && git commit` → `eac743d` (15 Sep 06:41Z)**. Akibatnya:
- **Tidak ada jejak commit-by-commit** — urutan harus direkonstruksi dari working-tree diff vs `315861e`, bukan `git log --oneline` (yang kosong untuk jendela ini).
- **Pelajaran implisit:** RAILS aturan sendiri (§6 post-hoc + DEFERRED) melarang bekerja berjam-jam tanpa commit ("jangan biarkan commits menumpuk local-only"). Jendela ini **melanggar aturan itu** — dan justru sebab utama #7 (bug eksekusi) lolos: tidak ada commit checkpoint untuk unit-test lindungi.

Urutan kejadian (rekonstruksi dari working-tree diff vs `315861e`; semua dalam SATU commit final `eac743d`, 2026-09-15T06:41:31Z):

1. **Audit + fix hardcode domain "sistem order makanan" di `prompts-v2.ts`** (cek juga `v2-engine/prompt-builder.ts`). Domain contoh makanan terlalu spesifik; ganti placeholder generik agar toko retail non-makanan tak bingung. *Commit: `eac743d`. Files: `apps/api/src/services/chat/prompts-v2.ts`, `apps/api/src/services/chat/v2-engine/prompt-builder.ts`.*
2. **Fix circuit-breaker gateway: double/multi-count per *percobaan provider*, bukan per pesan.** Counter `5` (per-pesan) over-trigger dengan rotasi N-provider karena backoff menghitung tiap *attempt* provider sebagai satu event. Pindahkan ke granularitas **per-message** (reset tiap `clientMsgId`). *Commit: `eac743d`. File: `apps/api/src/adapters/ai/llm-gateway.ts`. Precursor `552d489` (5 Sep, visibility AllProvidersCooldownError) — konteks, bukan fix ini.*
3. **Schema/normalisasi v2-engine — dua fase:**
   - **(a) SALAH, ditolak** — perlebar Zod `enum` untuk terima sinonem LLM (`ADD_TO_CART`/`add_to_cart`/`addtocart`). *Anti-pattern: enum bloat / drift* — takut LLM "mengajari" sinonem baru ke enum; Zod `enum` tak boleh jadi kamus sinonem. Tak ada commit terisolasi (ditimpa di working tree sebelum commit).
   - **(b) KOREKSI** — pisahkan **normalisasi sinonem** (`normalizeV2Output` di `engine-call.ts` / `mapV2ActionsToCartOps`) **sebelum** validasi; Skema tetap kecil (enum tidak melebar). Prinsip: *schema = gate ketat; sinonem = adapter layer di luar schema.* *Commit: `eac743d`. Files: `v2-engine/schema.ts`, `v2-engine/schema.test.ts`, `v2-engine/engine-call.ts`, `v2-engine/map-actions-to-cart-ops.ts`.*
4. **Tambah `UPDATE_SHIPPING_ADDRESS` sebagai action_type baru** (schema + mapper + handler). *Commit: `eac743d`. Files: `domain/types.ts`, `v2-engine/schema.ts`, `map-actions-to-cart-ops.ts`, `business/conversation.service.ts`.*
5. **Diagnosis provider testing gap + fix dashboard AI Providers:**
   - **Rasio:** provider `Kilo`/`Laguna` test-connection gagal diam-diam karena tak pakai `jsonMode` sama produksi → LLM kembali JSON tak ter-parse, test lapor "OK" sementara produksi error. *Commit: tidak ada (laporannya untracked belum commit).*
   - **Fix dashboard:** kolom *Last Used / Usage*, tombol *Test Production*, mobile layout break. *Commit: `eac743d`. Files: `apps/api/src/routes/admin/ai-providers.ts`, `apps/dashboard/src/pages/admin/AIProviders.tsx`.*
6. **Trim system prompt Inggris (v2-engine) ~22% lebih ringkas** (token cost + konsistensi; Inggris dipakai dev/debug, boleh ringkas). *Commit: `eac743d`. Files: `apps/api/src/services/chat/prompts-v2.ts`, `apps/api/src/services/chat/v2-engine/prompt-builder.ts`.*
7. **⚠️ AKTIVASI PERTAMA `v2RewriteMode='active'` global — dan TEMUAN KRITIS.** Setelah fix #1–#6, owner masuk ke mode *active* via `chat-engine-flip.sh`. **Temuan:** `proposed_actions` (ADD_TO_CART dsb.) **TIDAK PERNAH dieksekusi ke DB** sejak awal — `executeWaCartMutation` tidak dipanggil di active path. Seluruh balasan *"sudah ditambahkan ke keranjang"* adalah **klaim kosong, tidak ada OrderItem mutasi.** Bug ditemukan **dari pemakaian MANUAL owner** (screenshot UI "Totalin" placeholder "...", tidak ada order item bertambah), **bukan** dari test otomatis — 57 skenario trial VPS + seluruh unit test **LOLOS** meski bug ada. *Commit: `eac743d` (hanya mencatat temuan; fix-nya #8). File yang menyimpan bug: `business/conversation.service.ts` (active §5 tidak memanggil `executeWaCartMutation`).*

   > **📌 PELAJARAN PALING PENTING periode ini** (preseden §10.3 actionsRouter mount gap): *test otomatis hijau TIDAK menjamin eksekusi nyata terjadi, bila test-nya tidak verifikasi state DB.* `golden-dataset.test.ts` lama memakai `canned()`/`cannedV2()` (format lama) yang **bypass active §5 execute** → `parse_error` → tak sentuh DB, tapi tetap "pass." Solusi: migrasi ke `cannedV2Output` (schema sah) yang **melewati §5 execute** → baru verifikasi OrderItem tersimpan. Inilah sebab teknis kenapa #13 (golden 35/35) wajib SEBELUM #14 (re-aktivasi final) dianggap aman.

8. **Fix: sambungkan `proposed_actions` ke eksekusi nyata — REUSE, bukan rebuild.** `proposedActions → mapV2ActionsToCartOps → executeWaCartMutation → cartAuthority.executeOps`. Perbaiki semantik: `OPEN_CART` → `'view_cart'` (read, bukan modify); eskalasi (`CONTACT_ADMIN`) → `markHumanTakeover` + `eventBus.publish` (sebelumnya notifikasi merchant diam-diam tidak pernah terpicu karena `eventBus.publish` tidak dipanggil). *Commit: `eac743d`. Files: `business/conversation.service.ts`, `business/action-registry.ts`, `services/message-processor.service.ts`, `services/structured-message.mapper.ts`.*
9. **Fix reply-honesty gap kedua:** aksi belum ter-wire (dulu `CANCEL_ORDER`/`UPDATE_CART_QUANTITY` sebelum #11) pernah diklaim "berhasil" oleh LLM meski tidak dieksekusi — `mapV2ActionsToCartOps` lempar ke `skipped` tapi `buildResult` tetap `success`. Diperbaiki: aksi `skipped` (non-mutation / unsupported) → **balasan jujur** ("belum dapat memproses X, coba Y / hubungi CS"), bukan klaim `success`. *Commit: `eac743d`. Files: `business/conversation.service.ts`, `v2-engine/map-actions-to-cart-ops.ts`, `v2-engine/enrichment.ts` (`buildActiveSafeReply`).*
10. **Reconnect payment flow (QRIS/transfer/COD) — REUSE, bukan rebuild.** Audit membuktikan trigger lama (`message-processor.service.ts` `sendQrisFollowUp`; picu `result.source==='payment' && result.metadata?.qrisImageUrl`) terputus di jalur v2 karena active path tak menghasilkan `result.source==='payment'`. **Solusi:** reuse `fallbackService.getPaymentInfo(storeId)` (membaca `store.acceptsTransfer/acceptsQris/acceptsCod` + `qrisImageUrl` + `bankAccounts`; **COD hanya jika `acceptsCod`** per `DECISION-COD-SETTLEMENT-DEFERRED.md`) sebagai jembatan; active path memetakan `intent:'payment_inquiry'`/aksi `SHOW_PAYMENT_METHODS` → `getPaymentInfo` → `buildResult({source:PAYMENT, metadata:{…qrisImageUrl}})` → `sendQrisFollowUp`. Transfer/COD text via `gateway.sendMessage`; QRIS via `gateway.sendImage` (+ text-link fallback Fonnte). *Commit: `eac743d`. Files: `business/conversation.service.ts` (~L288 payment bridge), `business/fallback.service.ts` (`getPaymentInfo`), `services/message-processor.service.ts` (`sendQrisFollowUp` L416, trigger L321-322). Bukti: `wa-payment-v2-rewrite-e2e.test.ts` 6/6.*
11. **Wire penuh `CANCEL_ORDER`/`UPDATE_CART_QUANTITY`/`UPDATE_SHIPPING_ADDRESS`** (bukan cuma skema) — reuse eksekusi lama: `CANCEL_ORDER → orderService.cancelOrder`, `UPDATE_CART_QUANTITY → cartAuthority.updateQuantity`, `UPDATE_SHIPPING_ADDRESS → Order.shippingAddress` (via `handleUpdateShippingAddress` §5b). Dipilah dari mapper: `CANCEL_ORDER`/`UPDATE_SHIPPING_ADDRESS` **dikeluarkan dari `cartOps`** (proses terpisah ke handler khusus). *Commit: `eac743d`. Files: `business/action-registry.ts`, `business/cart-authority.ts`, `business/conversation.service.ts`. Bukti: `wa-cancel-qty-shipping-v2-active-e2e.test.ts` 5/5.*
12. **Fix mapper anti-hallucination — `mapV2ActionsToCartOps` tidak percaya `requires_validation` LLM untuk mutasi.** Sebelumnya cek `requires_validation===false` **pertama** (sebelum `MUTATION_ACTION_TYPES`), jadi hallucinated `requires_validation:false` pada `ADD_TO_CART` diskip → `cartOps=[]` → tidak dieksekusi. Diperbaiki: cek `MUTATION_ACTION_TYPES` **pertama**, **force-execute** (mengabaikan flag LLM) untuk `ADD_TO_CART`/`REMOVE_FROM_CART`/`UPDATE_CART_QUANTITY`; `requires_validation===false` hanya untuk read-only types (`OPEN_CART` dsb.). *Commit: `eac743d`. Files: `v2-engine/map-actions-to-cart-ops.ts` + `v2-engine/map-actions-to-cart-ops.test.ts` (9/9, termasuk anti-hallucination case). Prinsip: field boolean yang dikontrol LLM **tidak boleh** jadi gerbang keputusan mutasi keranjang.*
13. **Migrasi `golden-dataset.test.ts` 25/34 gagal → 35/35 hijau.** Penyebab 25/34: mock lama (`canned()`/`cannedV2()` bentuk `InterpreterResultV2` {acts/draft_cart_ops/…}) tidak cocp schema `V2EngineOutput` → `normalizeV2Output` `{success:false, parse_error}` → `buildActiveSafeReply` → tak ada side-effect DB → semua OrderItem/cart assertion merah. Migrasi ke `cannedV2Output` (schema sah) → `{success:true}` → melewati §5 execute → DB tercapai. + `Case 11 / P6-5/P6` (anti-hallucination: `ADD_TO_CART` `requires_validation:false` "oleh LLM" — STILL execute → OrderItem persisted, DB price). *Commit: `eac743d`. File: `apps/api/src/tests/golden-dataset.test.ts`.*
14. **Re-aktivasi FINAL `v2RewriteMode='active'` global** setelah #1–#13 diverifikasi (golden 35/35, chat 73/73, payment 6/6, cancel/qty/shipping 5/5, tsc 0, build 0, DB clean). Berbeda #7 (aktivasi pertama, rusak) — FINAL ini hanya setelah semua gate hijau. `chat-engine-flip.sh` menandai `chatEngine.v2RewriteMode='active'`; `reasoning.ts` tetap tidak ter-import (lihat entry 14 Sep). *Commit: `eac743d` = re-aktivasi final.*

Keputusan:
- Dokumentasikan 10–14 Sep sebagai **SATU commit final `eac743d`** (bukan per-event) — tidak ada commit intermediate yang bisa disebut. Rekonstruksi dari working-tree diff vs `315861e`.
- **#7 (aktivasi pertama + bug eksekusi) dicatat eksplisit, paling penting** — ditemukan manual owner, tidak oleh test; preseden §10.3.
- **#3 (schema) mencatat kedua fase**: percobaan enum-bloat yang SALAH (ditolak, tidak ada commit terisolasi) + koreksi normalisasi-terpisah.
- **#12 (mapper anti-hallucination) prinsip permanen**: `MUTATION_ACTION_TYPES` (daftar internal kode) = keputusan; `requires_validation` LLM = informatif saja, tidak mengunci mutasi.
- `reasoning.ts` tetap ada, tidak di-import active path; hapus permanen ditunda observasi (DEFERRED #40).

Alasan:
- **Transparansi vs. kebenaran:** tidak memaksakan hash per-event yang tidak ada — RAILS §10.2 melarang menyamar.
- **Sebab #7 lolos test:** golden/unit lama memakai mock format lama yang **bypass §5 execute** (parse_error, tak ada DB side-effect) → "pass" tapi tak pernah sentuh OrderItem. #13 (cannedV2Output melewati §5 execute) adalah sesi yang bisa meringkas ini. **Aturan emas baru tiap v2-test:** *test harus melewati active execute layer + verifikasi state DB, bukan hanya reply_text.*
- **Jendela 10–14 Sep juga melanggar kebijakan RAILS** ("jangan biarkan commits menumpuk local-only berjam-jam"). Catatan ini sebagai *post-mortem*: lain kali tiap phase (#1–#13) cukup commit terpisah agar bisa direkonstruksi per-event.
- **14→15 Sep:** murni sesi kerja lintas tengah malam UTC (owner baca metadata 14 Sep 11:56Z; `git commit eac743d` terjadi ketika sesi melintasi midnite UTC ke 15 Sep 06:41Z) — bukan bug jam.

Siapa yang setuju: owner (Panji) — rekonstrusi sesi 14 Sep 2026, di-commit `eac743d` (15 Sep 2026 06:41:31Z). Keputusan final (#14) disetujui hanya setelah gate hijau penuh (golden 35/35, chat 73/73, payment 6/6, cancel/qty/shipping 5/5, tsc=0, build=0, DB clean). Entri ini telah ditulis permanen ke RAILS.md (di antara entri 9 Sep dan 14 Sep); bukan draft lagi.
