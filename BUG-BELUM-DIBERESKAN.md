...
| III-9 | **`LEASE_FINAL_MS = 750` (action-registry.ts:22) adalah 750 MILIDETIK, bukan 750 detik** — kontrak PROJECT-CONTRACT-STRUCTURED-ACTIONS.md §6A.5 menargetkan lease 30–60 detik. Mismatch kemungkinan warisan implementasi P0 lama; ditemukan saat baca line untuk audit idempotency P7. Dampak: lease recovery claim macet terlalu singkat (750ms) → concurrency tinggi bisa claim ganda dalam jendela sempit, atau recovery CLAIMED terlalu agresif. **DI LUAR SCOPE P7** (RAILS §1.4) — jangan diperbaiki di P7. | apps/api/src/business/action-registry.ts:22; DOCS/CONTRACT/PROJECT-CONTRACT-STRUCTURED-ACTIONS.md §6A.5 | Low–Medium (idempotency edge) | Follow-up terpisah: naikkan ke 30–60 detik (atau nilai kontrak) + re-test claim/recovery. Catat di antrian, bukan blocker P7. **Post-P7 (19 Agu 2026):** P7 (range `0e6a5fa..b47f13c`) secara eksplisit TIDAK mengubah `LEASE_FINAL_MS` (tetap 750ms, per RAILS §1.4) — item ini tetap TERBUKA, dicatat di sini sebagai residual, bukan diperbaiki di P7. |
