...
| III-7 | **I11: kamus slang normalizer** (`toralin`→`total`) typo lolos ke tier total. | STATUS-V2:42 (normalizer.ts) | 🟡 **PARTIALLY STALE — audited 19 Agu 2026** | Klaim literal "toralin lolos" STALE — sudah di dictionary (`normalizer.ts:23`) + tested (`normalizer.test.ts:13`). NEW EDGE CASE: lookup **case-sensitive** (`normalizer.ts:153`, no `toLowerCase`) — varian kapital (`"Toralin brp"`) tidak match dictionary → lolos ke tier total tanpa normalisasi. Belum ada test varian kapital. Risiko LOW. Lihat §V (b). |
| III-8 | **I12: guard nama produk di normalizer**. | STATUS-V2:43 | 🟡 **MASIH RELEVAN, lebih luas dari test existing — audited 19 Agu 2026** | Test I12 (`normalizer.test.ts:17,29`) hanya cover 2 kasus 1-token lowercase — guard `fuzzyMatchProduct` (`normalizer.ts:120-129`) case-sensitive + per-token → nama produk **MULTI-KATA tidak ter-guard**. Bukti: `normalize('ada ready pack',['Ready Pack'])` → `"ada ada pack"` (produk berubah, melanggar I12). Caller kirim nama DB asli (`conversation.service.ts:616`). Risiko LOW–MEDIUM, nama produk termutasi sebelum tier match. Lihat §V (b). |
