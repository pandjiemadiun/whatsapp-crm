...
| 18 | `adapters.knowledge` and `adapters.storage` stub properties removed from `container.ts` exports (dead code, 0 references in running app). `adapters.llm` left intact per owner's separate intent (next task: ADAPTERS-LLM-CHAT-FIX). | 3 Sep 2026 | `3 Sep 2026 session` |
