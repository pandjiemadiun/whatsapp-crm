...
| 23 | Dua UI Magic Paste terpisah (ProductsPage.tsx untuk merchant, ConfirmCreateModal.tsx untuk admin) dengan fitur yang bisa diverge — root cause seluruh bug varian sesi 2-3 Sep 2026 (fitur dibangun di satu UI, tidak disadari UI lain juga perlu). Sekarang sudah disamakan manual untuk fitur varian, tapi risiko divergence berulang tetap ada untuk fitur mendatang | VARIANT-LIFECYCLE-FULL-AUDIT-AND-FIX, 3 Sep 2026 | Konsolidasi jadi 1 komponen bersama adalah refactor terpisah, tidak mendesak selama kedua UI disinkronkan manual tiap ada fitur baru | Kalau ada fitur magic-paste baru lagi ke depan — WAJIB cek dan terapkan ke KEDUA UI, atau prioritaskan konsolidasi saat itu |
| 21 | Dua UI magic-paste terpisah (merchant ProductsPage.tsx vs admin ConfirmCreateModal.tsx) — merchant UI belum punya editable variant editing meskipun admin UI sudah punya. Sudah diperbaiki di sesi ini, tetapi konsolidasi UI (satu komponen shared) masih belum dilakukan | VARIANT-LIFECYCLE-FULL-AUDIT-AND-FIX, 3 Sep 2026 | Perbaikan fungsional sudah dilakukan di merchant UI; konsolidasi arsitektur adalah pekerjaan terpisah yang tidak memblokir | Saat ada waktu luang untuk refactor — pertimbangkan extract ConfirmCreateModal pattern jadi shared component dipakai kedua UI |
