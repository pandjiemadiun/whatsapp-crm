...
| 23 | Admin/merchant UI duplication confirmed across multiple pairs (DUAL-UI-ARCHITECTURE-AUDIT, 3 Sep 2026): auth-level separation is intentional (AdminUser global vs Store tenant-scoped), but FEATURE-level implementation is duplicated per-pair instead of shared — causing drift risk. HIGH-risk pairs: (1) Magic Paste (ConfirmCreateModal vs ProductsPage) — drift CONFIRMED + fixed today; (2) Product CRUD (AdminProductsPage vs ProductsPage) — NOT yet confirmed drifted, but same structural risk. MEDIUM: Analytics, AI Config. LOW: GOWA/WhatsApp connect (genuinely different scope, low risk) | DUAL-UI-ARCHITECTURE-AUDIT, 3 Sep 2026 | Full consolidation of all pairs is a large refactor; owner + Claude agreed to prioritize the 2 HIGH-risk pairs first, not a big-bang rewrite | Product CRUD (AdminProductsPage vs ProductsPage) consolidation — next in queue after magic-paste. MEDIUM/LOW pairs — revisit only if a real drift bug surfaces in them |
