...
| 28 | Chat engine architecture overhaul — owner reports persistent "not connected" conversation quality (e.g. "minta total malah dibatalkan") despite 3+ rounds of refactor (P0-P8, B3/B4 tier fixes, golden dataset). Suspected root cause: keyword/pattern-based fallback tier chain (fallback.service.ts, 11 tiers) has repeatedly produced keyword-collision bugs (bayar/total, sop/ganti, shipping/pesan, 'mau' short-circuit — never fully resolved) — an inherent limitation of keyword matching for intent classification, not a series of isolated bugs. Additional factor: dual v1/v2 engine paths (per-store flag) add inconsistency risk. Owner explicit: token cost is NOT a constraint for this decision — recommend LLM-first intent classification (structured output) as primary router, keyword tiers demoted to emergency-fallback-only, and v1/v2 consolidated to one path | Owner request, 3 Sep 2026 — explicit "cari waktu yang tepat" (not now) | Major architecture change touching the most sensitive production path (live customer chat) — needs fresh full audit + careful phased migration, not a quick patch; currently robot occupied with N-provider rotation | After N-provider rotation + before/alongside Track B Wizard, or whenever owner is ready to dedicate a focused multi-session effort — owner to decide timing explicitly |
| 29 | **Status data klariﬁkasi diri** — seluruh toko di sistem adalah **dummy/test data** (termasuk store-4f4f67bd "Bengkel Didik"). **TIDAK ADA merchant produksi/asli yang terdaftar di DB.** Canary window §7.1 hanya bisa pakai test simulation (Fase 1), bukan percakapan customer asli — Fase 2 (30 real conversations) tertunda sampai merchant asli pertama terdaftar. | `PROJECT-CONTRACT-CHAT-ENGINE-V2-REWRITE.md` §7 (amendment 4 Sep 2026) | Fakta operasional, bukan task teknis — tapi berdampak pada go-live gate (§7.1 Fase 2 bergantung merchant asli) | Saat ada merchant asli pertama terdaftar dan onboarding selesai |
