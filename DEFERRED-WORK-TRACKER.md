...
| 24 | `products-routes.e2e.test.ts` tests #9/#11/#13 fail — `createProductSchema` kini wajib `weight`, tapi test lama tidak kirim field itu. Sama pola dengan item #19 (stale expectation, bukan bug produk) | VERIFY-PRODUCTDETAILPAGE-VARIANT-FIX, 3 Sep 2026 | Test expectation ketinggalan zaman | Kapan saja — tambahkan `weight: 1` ke request body 3 test itu |
...
| 25 | `ProductFormModal.tsx` — 0 referensi tersisa di manapun sejak digantikan `ProductForm.tsx` shared component. Kandidat hapus | PRODUCT-CRUD-CONSOLIDATION, 3 Sep 2026 | Pola sama `message.handler.ts` lama — laporan dulu, hapus saat siklus maintenance | Siklus maintenance komponen dashboard berikutnya |
